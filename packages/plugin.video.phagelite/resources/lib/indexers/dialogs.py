...
def language_invoker_choice(params):
	...
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.phagelite/addon.xml')
...
def addon_icon_choice(params):
	...
	from xml.dom.minidom import parse as mdParse
	large_image_url = 'https://raw.githubusercontent.com/phage-zn/phagelite.github.io/main/packages/addon_icons/%s'
...
def personallists_manager_choice(params):
	...
	list_type = params['list_type']
	all_lists = get_all_personal_lists(get_setting('phagelite.personal_list.list_sort', '0'))
	choices = []
...
def set_quality_choice(params):
	...
	fl = ['4K', '1080p', '720p', 'SD']
	q_setting = get_setting('phagelite.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('phagelite.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('phagelite.', ''), default_setting_values(setting_id)['setting_default'])
					button_action = get_setting('phagelite.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('phagelite.%s' % filter_setting_id).split(', ')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or kodi_utils.get_icon('phagelite')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('phagelite.%s' % setting), default_setting_values(setting)['setting_default']
...
def options_menu_choice(params, meta=None):
	...
	single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_phagelite', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
