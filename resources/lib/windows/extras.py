...
class Extras(BaseDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.items_list_ids = (
            Extras.recommended_id,
            Extras.related_id,
            Extras.more_like_this_id,
            Extras.ai_similar_id,
            Extras.year_id,
            Extras.genres_id,
            Extras.networks_id,
    ...
    def onAction(self, action):
        ...
        if action == self.info_action:
            ...
            if not focus_id in self.items_list_ids:
                return
            ...
            meta = function(
                "tmdb_id",
                chosen_listitem.getProperty("tmdb_id"),
                self.tmdb_api_key,
                self.mpaa_region,
                self.current_date,
            )
            ...
            focus_id = self.getFocusId()
            if focus_id == Extras.cast_id:
                person_name = self.get_listitem(focus_id).getProperty(
                    self.item_action_dict[focus_id]
                )
                return people.person_search(person_name)
            elif focus_id == Extras.in_lists_id:
                kodi_utils.show_busy_dialog()
                try:
                    list_item, position = (
                        self.get_listitem(focus_id),
                        self.get_position(focus_id),
                    )
                    chosen = self.get_attribute(
                        self, list_item.getProperty(self.item_action_dict[focus_id])
                    )[self.get_position(focus_id)]
                    user, list_slug = (
                        chosen["user"]["ids"]["slug"],
                        chosen["ids"]["slug"],
                    )
                    function, new_value = (
                        (trakt_api.trakt_like_a_list, "true")
                        if list_item.getProperty("liked_status") == "false"
                        else (trakt_api.trakt_unlike_a_list, "false")
                    )
                    new_value = (
                        "true"
                        if list_item.getProperty("liked_status") == "false"
        ...
        if action in self.selection_actions:
            ...
            if self.control_id in self.items_list_ids:
                ...
                return window_manager(self)
            elif self.control_id == Extras.cast_id:
                self.set_current_params()
                self.new_params = {
                ...
                return window_manager(self)
            elif self.control_id == Extras.videos_id:
                self.set_current_params(set_starting_position=False)
                self.window_player_url = (
                    "plugin://plugin.video.youtube/play/?video_id=%s" % chosen_var
                )
                return window_player(self)
            elif self.control_id in self.text_list_ids:
                if self.control_id == Extras.parentsguide_id:
                    return self.show_text_media(text=chosen_var)
                else:
                    return self.select_item(
                        self.control_id,
                        self.show_text_media(
                            text=self.get_attribute(self, chosen_var),
    ...
    def make_ratings(self, win_prop=4000):
        ...
        for prop, _id in (
            ...
            if rating in ("", "%"):
                continue
            if prop == "tmdb" and not active_extra_ratings:
                continue
            (
                self.setProperty("%s_rating" % prop, "true"),
                self.set_label(win_prop + _id, rating),
                self.set_image(
                    win_prop + 100 + _id, "bacterio_flags/ratings/%s" % icon
                ),
            )
    ...
    def make_plot_and_tagline(self):
        ...
        if Extras.plot_id in self.enabled_lists:
            self.setProperty("plot_enabled", "true")

    def make_cast(self):
        if not Extras.cast_id in self.enabled_lists:
            return
    ...
    def make_recommended(self):
        if not Extras.recommended_id in self.enabled_lists:
            return
        try:
            function = (
                tmdb_api.tmdb_movies_recommendations
                if self.media_type == "movie"
                else tmdb_api.tmdb_tv_recommendations
            )
    ...
    def make_related(self):
        if not Extras.related_id in self.enabled_lists:
            return
        ...
        def builder(position, item):
            try:
                ...
                listitem.setProperty(
                    "thumbnail", details["poster"] or self.empty_poster
                )
    ...
    def make_more_like_this(self):
        if not Extras.more_like_this_id in self.enabled_lists:
            return
    ...
    def make_ai_similar(self):
        if not Extras.ai_similar_id in self.enabled_lists:
            return
    ...
    def make_reviews(self):
        if not Extras.reviews_id in self.enabled_lists:
            return
    ...
    def make_comments(self):
        if not Extras.comments_id in self.enabled_lists:
            return
    ...
    def make_in_lists(self):
        if not Extras.in_lists_id in self.enabled_lists:
            return
        ...
        def builder():
            for count, item in enumerate(self.all_in_lists, 1):
                try:
                    ...
                    likes_insert = (
                        "[CR]%s %s" % (likes, "Likes" if likes > 1 else "Like")
                        if likes
                        else ""
                    )
                    listitem.setProperty(
                        ...
                        % (
                            count,
                            batch_replace(item["name"].upper(), replacements),
                            likes_insert,
                            item["user"]["ids"]["slug"],
                            item["item_count"],
                        ),
        ...
        try:
            ...
            self.all_in_lists = trakt_api.trakt_lists_with_media(
                self.media_type, self.imdb_id
            )
            item_list = list(builder())
    ...
    def make_trivia(self):
        if not Extras.trivia_id in self.enabled_lists:
            return
    ...
    def make_blunders(self):
        if not Extras.blunders_id in self.enabled_lists:
            return
    ...
    def make_parentsguide(self):
        if not Extras.parentsguide_id in self.enabled_lists:
            return
        ...
        def builder():
            for item in data:
                try:
                    ...
                    icon = Extras.parentsguide_icons[name]
    ...
    def make_videos(self):
        if not Extras.videos_id in self.enabled_lists:
            return
        ...
        def _sort_trailers(trailers):
            official_trailers = [
                ...
                if i["official"]
                and i["type"] == "Trailer"
                and "official trailer" in i["name"].lower()
            ]
            other_official_trailers = [
                i
                for i in trailers
                if i["official"]
                and i["type"] == "Trailer"
                and not i in official_trailers
            ]
            other_trailers = [
                i
                for i in trailers
                if i["type"] == "Trailer"
                and not i in official_trailers
                and not i in other_official_trailers
            ]
        ...
        def builder():
            for item in all_trailers:
                try:
                    ...
                    listitem.setProperty("name", item["name"])
                    listitem.setProperty(
                        "thumbnail", "https://img.youtube.com/vi/%s/0.jpg" % key
                    )
    ...
    def make_year(self):
        if not Extras.year_id in self.enabled_lists:
            return
        try:
            function = (
                tmdb_api.tmdb_movies_year
                if self.media_type == "movie"
                else tmdb_api.tmdb_tv_year
            )
    ...
    def make_genres(self):
        if not Extras.genres_id in self.enabled_lists:
            return
        try:
            function, genre_list = (
                (tmdb_api.tmdb_movies_genres, movie_genres())
                if self.media_type == "movie"
                else (tmdb_api.tmdb_tv_genres, tvshow_genres())
            )
            genre_list = ",".join(
                [i["id"] for i in genre_list if i["name"] in self.genre]
            )
            data = self.remove_current_tmdb_mediaitem(
                function(genre_list, 1)["results"]
            )
            item_list = list(self.make_tmdb_listitems(data))
    ...
    def make_network(self):
        if not Extras.networks_id in self.enabled_lists:
            return
        try:
            network = self.meta_get("studio")[0]
            network_list = (
                tmdb_api.tmdb_company_id(network)["results"]
                if self.media_type == "movie"
                else networks()
            )
            network_id = next(i["id"] for i in network_list if i["name"] == network)
            function = (
                tmdb_api.tmdb_movies_companies
                if self.media_type == "movie"
                else tmdb_api.tmdb_tv_networks
            )
            data = self.remove_current_tmdb_mediaitem(
                function(network_id, 1)["results"]
            )
            item_list = list(self.make_tmdb_listitems(data))
    ...
    def make_collection(self):
        if self.media_type != "movie":
            return
        if not Extras.collection_id in self.enabled_lists:
            return
        try:
        ...
        try:
            ...
            item_list = list(
                self.make_tmdb_listitems(
                    sorted(data["parts"], key=lambda k: k["release_date"] or "2050")
                )
            )
            self.setProperty("more_from_collection.name", data["title"])
            self.setProperty(
                "more_from_collection.overview", data["plot"] or data["title"]
            )
            self.setProperty(
                "more_from_collection.poster", data["poster"] or self.empty_poster
            )
    ...
    def get_finish(self, percent_watched):
        ...
        if self.duration_data:
            label = (
                "Finish Rewatching" if percent_watched == "100" else "Finish Watching"
            )
    ...
    def get_duration(self):
        ...
        if self.duration_data:
            ...
            if minute:
                time_str += "%s%sm" % (
                    " " if hour else "",
                    "%d" % minute if minute < 10 else "%02d" % minute,
                )
        return time_str
    ...
    def get_next_episode(self):
        ...
        try:
            try:
                ...
                ep_data = next(
                    (i for i in ep_list if i["media_ids"]["tmdb"] == self.tmdb_id), None
                )
            ...
            watched_info = watched_status.watched_info_episode(
                self.tmdb_id, watched_status.get_database(settings.watched_indicators())
            )
            nextep_season, nextep_episode = watched_status.get_next(
                orig_season, orig_episode, watched_info, season_data, nextep_content
            )
            if not nextep_season:
                return
            episodes_data = episodes_meta(nextep_season, self.meta)
            item = next(
                (i for i in episodes_data if i["episode"] == nextep_episode), None
            )
            item_get = item.get
            episode_date, premiered = adjust_premiered_date(
                item_get("premiered"), settings.date_offset()
            )
            if episode_date and self.current_date >= episode_date:
                self.nextep_season, self.nextep_episode = nextep_season, nextep_episode
                value = "Next Episode: S%.2dE%.2d" % (
                    self.nextep_season,
                    self.nextep_episode,
                )
        except:
    ...
    def make_tmdb_listitems(self, data):
        ...
        for item in data:
            try:
                ...
                if poster_path:
                    thumbnail = "https://image.tmdb.org/t/p/%s%s" % (
                        "w300",
                        poster_path,
                    )
                else:
                    thumbnail = self.empty_poster
    ...
    def show_text_media(self, text, poster=None, current_index=None):
        return self.open_window(
            ("windows.extras", "ShowTextMedia"),
            "textviewer_media.xml",
            text=text,
        ...
        if meta:
            text = "[B]  •  [/B]".join(
                [
                    ...
                    for i in (
                        meta.get("year"),
                        str(round(meta.get("rating"), 1))
                        if meta.get("rating") not in (0, 0.0, None)
                        else None,
                        meta.get("mpaa"),
                    )
                    if i
            ...
            text, poster = (
                dialogs.media_extra_info_choice(
                    {"media_type": self.media_type, "meta": self.meta}
                ),
                self.poster,
            )
    ...
    def show_genres(self):
        ...
        self.close_all()
        mode, action = (
            ("build_movie_list", "tmdb_movies_genres")
            if self.media_type == "movie"
            else ("build_tvshow_list", "tmdb_tv_genres")
        )
        self.selected = self.folder_runner(
            {"mode": mode, "action": action, "key_id": genre_id}
        )
        self.close()

    def show_keywords(self):
        keyword_id = dialogs.keywords_choice(
            {"media_type": self.media_type, "meta": self.meta}
        )
        if not keyword_id:
            return
        self.close_all()
        mode, action = (
            ("build_movie_list", "tmdb_movie_keyword_results")
            if self.media_type == "movie"
            else ("build_tvshow_list", "tmdb_tv_keyword_results")
        )
        self.selected = self.folder_runner(
            {"mode": mode, "action": action, "key_id": keyword_id}
        )
        self.close()
    ...
    def play_random_episode(self):
        self.close_all()
        function = dialogs.random_choice(
    ...
    def show_recommended(self):
        self.close_all()
        mode, action = (
            ("build_movie_list", "tmdb_movies_recommendations")
            if self.media_type == "movie"
            else ("build_tvshow_list", "tmdb_tv_recommendations")
        )
        self.selected = self.folder_runner(
            {
                "mode": mode,
                "action": action,
                "key_id": self.tmdb_id,
                "name": "Recommended based on %s" % self.title,
            }
        )
        self.close()
        ...
        self.selected = self.folder_runner(
            {
                ...
                "key_id": self.imdb_id,
                "name": "More Like This based on %s" % self.title,
            }
        )
        self.close()
    ...
    def show_in_trakt_lists(self):
        ...
        self.selected = self.folder_runner(
            {
                "mode": "trakt.list.in_trakt_lists",
                "media_type": media_type,
                "imdb_id": self.imdb_id,
                "category_name": "%s In Trakt Lists" % self.title,
            }
        )
        self.close()
        ...
        return dialogs.trakt_manager_choice(
            {
                "tmdb_id": self.tmdb_id,
                "imdb_id": self.imdb_id,
                "tvdb_id": self.meta_get("tvdb_id", "None"),
                "media_type": self.media_type,
                "icon": self.poster,
            }
        )
    ...
    def show_tmdb_manager(self):
        return dialogs.tmdblists_manager_choice(
            {
    ...
    def show_favorites_manager(self):
        return dialogs.favorites_manager_choice(
            {
                "media_type": self.media_type,
                "tmdb_id": str(self.tmdb_id),
                "title": self.title,
    ...
    def assign_buttons(self):
        setting_id_base = "bacterio.extras.%s.button" % self.media_type
    ...
    def set_current_params(self, set_starting_position=True):
        ...
        if set_starting_position:
            self.current_params["starting_position"] = [
                self.control_id,
                self.get_position(self.control_id),
            ]

    def set_starting_constants(self, kwargs):
        self.meta = kwargs.get("meta")
        self.meta_get = self.meta.get
        self.media_type, self.options_media_type = (
            self.meta_get("mediatype"),
            kwargs.get("options_media_type"),
        )
        ...
        self.folder_runner = (
            ...
            if self.is_external == "true"
            else kodi_utils.container_update
        )
        self.enabled_lists, self.enable_scrollbars = (
            settings.extras_enabled_menus(),
            settings.extras_enable_scrollbars(),
        )
        self.tmdb_api_key, self.omdb_api, self.mpaa_region = (
            settings.tmdb_api_key(),
            settings.omdb_api_key(),
            settings.mpaa_region(),
        )
        self.display_extra_ratings = (
            self.imdb_id
            and self.omdb_api not in ("empty_setting", "")
            and settings.extras_enable_extra_ratings()
        )
        self.title, self.year, self.rootname = (
            self.meta_get("title"),
            str(self.meta_get("year")),
            self.meta_get("rootname"),
        )
        ...
        self.rating = (
            str(round(self.meta_get("rating"), 1))
            if self.meta_get("rating") not in (0, 0.0, None)
            else None
        )
        self.mpaa, self.genre, self.network = (
            self.meta_get("mpaa"),
            self.meta_get("genre"),
            self.meta_get("studio") or "",
        )
        self.status, self.duration_data = (
            self.extra_info_get("status", "").replace(" Series", ""),
            int(float(self.meta_get("duration")) / 60),
        )
    ...
    def set_properties(self):
        ...
        self.setProperty(
            "display_extra_ratings", "true" if self.display_extra_ratings else "false"
        )
    ...
    def make_status_infoline(self):
        ...
        if self.media_type == "tvshow" and self.status == "Returning":
            ...
            if next_aired_date:
                status_str = "%s %s" % (
                    self.status,
                    adjust_premiered_date(next_aired_date, settings.date_offset())[
                        0
                    ].strftime("%d %B %Y"),
                )
        return status_str
    ...
    def make_stinger_dialog(self):
        ...
        if self.media_type == "movie":
            ...
            if stinger_keys:
                ...
                stinger_dialog = {
                    1: "%s Credits Stinger",
                    2: "%s & %s Credits Stinger",
                }[len(stinger_names)] % stinger_names
    ...
    def set_infoline2(self):
        if self.media_type == "movie":
            percent_watched = watched_status.get_progress_status_movie(
            ...
            if not percent_watched:
                try:
                    percent_watched = (
                        "100"
                        if watched_status.get_watched_status_movie(
                            watched_status.watched_info_movie(), str(self.tmdb_id)
                        )
                        == 1
                        else "0"
                    )
                ...
                if not percent_watched:
                    percent_watched = 0
            line2 = "[B]  •  [/B]".join(
                [self.get_progress(percent_watched), self.get_finish(percent_watched)]
            )
        else:
            line2 = "[B]  •  [/B]".join(
                [
    ...
    def youtube_installed_check(self):
        if not self.addon_installed("plugin.video.youtube"):
            return False
        if not self.addon_enabled("plugin.video.youtube"):
            return False
        return True

    def close_all(self):
        kodi_utils.clear_property("bacterio.window_stack")
...
class ShowTextMedia(BaseDialog):
    ...
    def update_text(self, direction=None):
        if direction == "previous":
            self.position = (
                self.position - 1 if self.position > 0 else self.len_text - 1
            )
        elif direction == "next":
            self.position = (
                0 if self.position == self.len_text - 1 else self.position + 1
            )
        self.set_text(
            2001, self.text[self.position] if self.text_is_list else self.text
        )
        if self.text_is_list:
            if self.position == 0:
