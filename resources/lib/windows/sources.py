...
class SourcesResults(BaseDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.prerelease_values, self.prerelease_key = (
            ("CAM", "SCR", "TELE"),
            "CAM/SCR/TELE",
        )
        ...
        self.info_quality_dict = {
            ...
            "1080p": get_icon("flag_1080p", "flags"),
            "720p": get_icon("flag_720p", "flags"),
            "sd": get_icon("flag_sd", "flags"),
            "cam": get_icon("flag_sd", "flags"),
            "tele": get_icon("flag_sd", "flags"),
            "scr": get_icon("flag_sd", "flags"),
        }
        self.make_items()
    ...
    def filter_action(self, action):
        ...
        if action in self.selection_actions:
            ...
            filter_type, filter_value = (
                chosen_listitem.getProperty("filter_type"),
                chosen_listitem.getProperty("filter_value"),
            )
            if filter_type in ("quality", "provider"):
                if filter_value == self.prerelease_key:
                    filtered_list = [
                        i
                        for i in self.item_list
                        if i.getProperty(filter_type) in self.prerelease_values
                    ]
                else:
                    filtered_list = [
                        i
                        for i in self.item_list
                        if i.getProperty(filter_type) == filter_value
                    ]
            elif filter_type == "special":
                if filter_value == "title":
                    keywords = kodi_dialog().input(
                    ...
                    filtered_list = [
                        i
                        for i in self.item_list
                        if all(x in i.getProperty("name") for x in choice)
                    ]
                elif filter_value == "extraInfo":
                    filters = source_filters()
                    list_items = [
                        {"line1": item[0], "icon": self.poster} for item in filters
                    ]
                    kwargs = {
                    ...
                    filtered_list = [
                        i
                        for i in self.item_list
                        if all(x in i.getProperty("extraInfo") for x in choice)
                    ]
                elif filter_value == "showuncached":
                    filtered_list = self.make_items(self.uncached_results)
                else:  # cache_check_rescrape
                    self.selected = (
                        "cache_change_rescrape",
                        "false" if self.external_cache_check else "true",
                    )
                    return self.close()
            if not filtered_list:
                return ok_dialog(text="No Results")
    ...
    def onAction(self, action):
        ...
        if action == self.info_action:
            ...
            if (
                self.prescrape
                and chosen_listitem.getProperty("perform_full_search") == "true"
            ):
            ...
            if choice:
                ...
                if choice == "rd_cloud_delete":
                    ...
                    function = (
                        rd_api.delete_torrent
                        if source["cache_type"] == "torrent"
                        else rd_api.delete_download
                    )
    ...
    def make_items(self, filtered_list=None):
        def builder(count, item):
            try:
                ...
                scrape_provider, source, quality, name = (
                    get("scrape_provider"),
                    get("source"),
                    get("quality", "SD"),
                    get("display_name"),
                ...
                if self.episode_group_label:
                    extraInfo = "%s | %s" % (self.episode_group_label, extraInfo)
                if not extraInfo:
                    extraInfo = "N/A"
                if scrape_provider == "external":
                    ...
                    if "Uncached" in item["cache_provider"]:
                        if "seeders" in item:
                            set_properties(
                                {
                                    "source_type": "UNCACHED (%d SEEDERS)"
                                    % get("seeders", 0)
                                }
                            )
                        else:
                            set_properties({"source_type": "UNCACHED"})
                        ...
                        if pack:
                            set_properties(
                                {"source_type": "%s [B]PACK[/B]" % cache_flag}
                            )
                        else:
                            set_properties({"source_type": "%s" % cache_flag})
                    ...
                    set_properties(
                        {
                            "highlight": self.info_highlights_dict[key],
                            "source_type": "DIRECT",
        ...
        try:
            ...
            if filtered_list:
                threads = TaskPool().tasks_enumerate(
                    builder, filtered_list, min(len(filtered_list), max_threads())
                )
                ...
                return item_list
            threads = TaskPool().tasks_enumerate(
                builder, self.results, min(len(self.results), max_threads())
            )
            [i.join() for i in threads]
            item_list.sort(key=lambda k: k[1])
    ...
    def make_filter_items(self):
        def builder(count, item):
            ...
            listitem.setProperties(
                {"label": item[0], "filter_type": item[1], "filter_value": item[2]}
            )
            self.filter_list.append((listitem, count))

        duplicates = set()
        qualities = [
            i.getProperty("quality")
            for i in self.item_list
            if not (
                i.getProperty("quality") in duplicates
                or duplicates.add(i.getProperty("quality"))
            )
            and not i.getProperty("quality") == ""
        ]
        if any(i in self.prerelease_values for i in qualities):
            qualities = [i for i in qualities if not i in self.prerelease_values] + [
                self.prerelease_key
            ]
        qualities.sort(key=("4K", "1080P", "720P", "SD", "CAM/SCR/TELE").index)
        quality_totals = {
            i: len([x for x in self.item_list if x.getProperty("quality") == i])
            for i in qualities
        }
        if "CAM/SCR/TELE" in qualities:
            quality_totals["CAM/SCR/TELE"] = len(
                [
                    i
                    for i in self.item_list
                    if i.getProperty("quality") in self.prerelease_values
                ]
            )
        duplicates = set()
        providers = [
            i.getProperty("provider")
            for i in self.item_list
            if not (
                i.getProperty("provider") in duplicates
                or duplicates.add(i.getProperty("provider"))
            )
            and not i.getProperty("provider") == ""
        ]
        provider_totals = {
            i: len([x for x in self.item_list if x.getProperty("provider") == i])
            for i in providers
        }
        ...
        qualities = [
            (
                "Show [B]%s[/B] Only | [B]%d[/B] Results" % (i, quality_totals[i]),
                "quality",
                i,
            )
            for i in qualities
        ...
        providers = [
            (
                "Show [B]%s[/B] Only | [B]%d[/B] Results" % (i, provider_totals[i]),
                "provider",
                i,
            )
            for i in providers
        ...
        data = []
        if cache_functions_debrid:
            data.append(
                (
                    "Rescrape with External Cache Check [B]%s[/B]"
                    % ("OFF" if self.external_cache_check else "ON"),
                    "special",
                    "cache_check_rescrape",
                )
            )
        if self.uncached_results:
            data.append(
                (
                    "Show [B]Uncached[/B] Only | [B]%d[/B] Results"
                    % len(self.uncached_results),
                    "special",
                    "showuncached",
                )
            )
        ...
        data.extend(
            [
                ("Filter by [B]Title[/B]...", "special", "title"),
                ("Filter by [B]Info[/B]...", "special", "extraInfo"),
            ]
        )
        self.filter_list = []
        threads = TaskPool().tasks_enumerate(
            builder, data, min(len(data), max_threads())
        )
        [i.join() for i in threads]
    ...
    def set_properties(self):
        ...
        self.setProperty(
            "filters_ignored", "| Filters Ignored" if self.filters_ignored else ""
        )

    def set_poster(self):
        if self.window_id == 2000:
    ...
    def context_menu(self, item):
        ...
        item_get = item.get
        item_id, name, magnet_url, info_hash = (
            item_get("id", None),
            item_get("name"),
            item_get("url", "None"),
            item_get("hash", "None"),
        )
        provider_source, scrape_provider, cache_provider = (
            item_get("source"),
            item_get("scrape_provider"),
            item_get("cache_provider", "None"),
        )
        ...
        if provider_source == "torrent":
            browse_pack_params = {
                "mode": "debrid.browse_packs",
                "provider": cache_provider,
                "name": name,
                "magnet_url": magnet_url,
                "info_hash": info_hash,
            }
            if cache_provider != "EasyDebrid":
                add_magnet_to_cloud_params = {
                    "mode": "manual_add_magnet_to_cloud",
                    "provider": cache_provider,
                    "magnet_url": magnet_url,
        ...
        if add_magnet_to_cloud_params:
            choices_append(("Add to Cloud", add_magnet_to_cloud_params))
        if browse_pack_params:
            choices_append(("Browse", browse_pack_params))
        if down_pack_params:
            choices_append(("Download Pack", down_pack_params))
        if down_file_params:
            choices_append(("Download File", down_file_params))
        if provider_source == "rd_cloud":
            choices_append(("Delete from RD Cloud", "rd_cloud_delete"))
...
class SourcesPlayback(BaseDialog):
    ...
    def onAction(self, action):
        ...
        elif action == self.right_action and self.window_mode == "resolver":
            self.skip_resolve = True
    ...
    def set_scraper_properties(self):
        ...
        fanart, clearlogo = (
            self.meta_get("fanart") or self.addon_fanart,
            self.meta_get("clearlogo") or "",
        )
...
class SourcesChoice(BaseDialog):
    ...
    def make_items(self):
        ...
        for item in [
            ("List", get_icon("results_list", "results")),
            ("Rows", get_icon("results_row", "results")),
