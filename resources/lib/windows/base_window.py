...
def window_manager(obj):
	...
	def monitor():
		timer = 0
		while not kodi_utils.get_property('bacterio.window_loaded') == 'true' and timer <= 5:
			kodi_utils.sleep(50)
			timer += 0.05
	...
	def get_stack():
		try: window_stack = json.loads(kodi_utils.get_property('bacterio.window_stack'))
		except: window_stack = []
		return window_stack
...
class FontUtils:
	def execute_custom_fonts(self):
		...
		skin_files = kodi_utils.list_dirs(kodi_utils.translate_path('special://home/addons/plugin.video.bacterio/resources/skins/Default/1080i/'))[1]
	...
	def skin_change_check(self):
		...
		if self.current_skin != kodi_utils.get_property('bacterio.current_skin') or self.current_font != kodi_utils.get_property('bacterio.current_font'): return True
		return False
	...
	def replace_font(self, window, replacement_values):
		file = kodi_utils.translate_path('special://home/addons/plugin.video.bacterio/resources/skins/Default/1080i/' + window)
