...
def window_manager(obj):
    def close():
        obj.close()
        kodi_utils.clear_property("bacterio.window_loaded")
        kodi_utils.clear_property("bacterio.window_stack")

    def monitor():
        timer = 0
        while (
            not kodi_utils.get_property("bacterio.window_loaded") == "true"
            and timer <= 5
        ):
            kodi_utils.sleep(50)
            timer += 0.05
    ...
    def add_to_stack(params):
        ...
        kodi_utils.set_property("bacterio.window_stack", json.dumps(window_stack))
...
def window_player(obj):
    ...
    try:
        ...
        if "plugin.video.youtube" in window_player_url:
            if not kodi_utils.addon_installed(
                "plugin.video.youtube"
            ) or not kodi_utils.addon_enabled("plugin.video.youtube"):
                return
        kodi_utils.clear_property("bacterio.window_loaded")
        current_params = obj.current_params
...
class BaseDialog(xbmcgui.WindowXMLDialog):
    ...
    def make_contextmenu_item(self, label, action, params):
        ...
        cm_item.set_properties(
            {"label": label, "action": action % self.build_url(params)}
        )
        return cm_item
    ...
    def get_home_property(self, prop):
        return kodi_utils.get_property("bacterio.%s" % prop)
...
class FontUtils:
    def execute_custom_fonts(self):
        ...
        if not self.skin_change_check():
            return
        replacement_values, skin_font_xml = [], ""
        replacement_values_append = replacement_values.append
        skin_folder = self.get_skin_folder()
        if skin_folder:
            skin_font_xml = kodi_utils.translate_path(
                "special://skin/%s/Font.xml" % skin_folder
            )
        if skin_font_xml:
            self.skin_font_info = (
                self.get_font_info(skin_font_xml) or self.default_font_info()
            )
        else:
            self.skin_font_info = self.default_font_info()
        for item in (
            (21, False, "font10"),
            (26, False, "font12"),
            (30, False, "font13"),
        ...
        skin_files = kodi_utils.list_dirs(
            kodi_utils.translate_path(
                "special://home/addons/plugin.video.bacterio/resources/skins/Default/1080i/"
            )
        )[1]
        for item in skin_files:
            self.replace_font(item, replacement_values)
        kodi_utils.set_property("bacterio.current_skin", self.current_skin)
        kodi_utils.set_property("bacterio.current_font", self.current_font)
    ...
    def get_skin_folder(self):
        ...
        try:
            skin_folder = (
                mdParse(kodi_utils.translate_path("special://skin/addon.xml"))
                .getElementsByTagName("extension")[0]
                .getElementsByTagName("res")[0]
                .getAttribute("folder")
            )
            if not skin_folder:
                s_folder = kodi_utils.list_dirs(
    ...
    def skin_change_check(self):
        self.current_skin, self.current_font = (
            kodi_utils.current_skin(),
            kodi_utils.jsonrpc_get_system_setting("lookandfeel.font", "Default"),
        )
        if self.current_skin != kodi_utils.get_property(
            "bacterio.current_skin"
        ) or self.current_font != kodi_utils.get_property("bacterio.current_font"):
            return True
        return False

    def match_font(self, size, bold, fallback):
        font_tag = "BACTERIO_%s%s" % (size, "_BOLD" if bold else "")
        size_range = range(int(size * 0.75), int(size * 1.25))
        compatibility_range = range(int(size * 0.50), int(size * 1.50))
        compatibility_fonts = [
            i["name"]
            for i in self.skin_font_info
            if i["name"] == fallback and i["size"] in compatibility_range
        ]
        if compatibility_fonts:
            return (font_tag, compatibility_fonts[0])
        sized_fonts = [i for i in self.skin_font_info if i["size"] in size_range]
        if not sized_fonts:
            return fallback
        fonts = (
            [i for i in sized_fonts if i["bold"] == bold and not i["extra_styles"]]
            or [i for i in sized_fonts if i["bold"] == bold]
            or [i for i in sized_fonts if not i["extra_styles"]]
            or sized_fonts
        )
        return (
            font_tag,
            [
                ...
                for i in fonts
                if i["size"]
                == min([i["size"] for i in fonts], key=lambda k: abs(k - size))
            ][0],
        )
    ...
    def get_font_info(self, skin_font_xml):
        ...
        try:
            ...
            for item in font_element:
                ...
                if not extra_styles:
                    extra_styles = any(item in name_compare for item in extras_keys)
                results_append(
                    {
                        "name": name,
                        "size": size,
    ...
    def default_font_info(self):
        return [
            {"name": "font10", "size": 21, "bold": False, "extra_styles": False},
