...
class EasyNewsAPI:
    def __init__(self):
        ...
        self.username = get_setting("bacterio.easynews_user", "empty_setting")
        self.password = get_setting("bacterio.easynews_password", "empty_setting")
    ...
    def search(self, query, expiration=48):
        ...
        return cache_object(
            self._process_search, string, url, json=False, expiration=expiration
        )
    ...
    def usage_info(self):
        ...
        try:
            ...
            usage_info = parseDOM(
                usage_html, "div", attrs={"class": "table-responsive"}
            )
    ...
    def process_image_files(self, files):
        def _process():
            ...
            for item in files:
                try:
                    post_hash, size, group, post_title, ext = (
                        item["0"],
                        item["4"],
                        item["9"],
                        item["10"],
                    ...
                    if ext == ".gif":
                        continue
                    if "type" in item and item["type"].upper() != "IMAGE":
                        continue
                    elif "virus" in item and item["virus"]:
                        continue
                    if any(i in group for i in ["comic", "fake", "erotica"]):
                        continue
                    url_add = quote(
                        "/%s/%s/%s%s/%s%s"
                    ...
                    file_dl = (
                        down_url + url_add + "|Authorization=%s" % self.auth_quoted
                    )
                    thumbnail = "https://th.easynews.com/thumbnails-%s/sm-%s.jpg" % (
                        post_hash[0:3],
                        post_hash,
                    )
                    result = {
                        "name": "%s_%s_%02d" % (self.query, post_title, count),
                        "fullsize": size,
        ...
        download_url = "https://%s:%s@members.easynews.com/dl" % (
            quote(self.username),
            quote(self.password),
        )
    ...
    def _translate_search(self, search_type="VIDEO"):
        video_extensions = (
            "m4v, 3g2, 3gp, nsv, tp, ts, ty, pls, rm, rmvb, mpd, ifo, mov, qt, divx, xvid, bivx, vob, nrg, img, iso, udf, pva, wmv, asf, asx, ogm, m2v, avi, bin, dat,"
    ...
    def _get(self, url, params={}):
        ...
        try:
            response = session.get(url, params=params, headers=headers, timeout=20).text
        except:
            return None
        try:
    ...
    def resolve_easynews(self, url_dl, use_non_seekable=False):
        ...
        if not response.ok:
            return None
        if use_non_seekable:
            resolved_link = response.url + "|seekable=0"
        else:
