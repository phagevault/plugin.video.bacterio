...
class EasyDebridAPI:
    ...
    def _post(self, url, params=None, json=None, data=None):
        ...
        response = session.post(
            url, params=params, json=json, data=data, headers=self.headers(), timeout=20
        )
        return response.json()
        ...
        try:
            ...
            if season:
                ...
                torrent_files = [
                    i
                    for i in torrent_files
                    if not any(x in i["filename"] for x in extras_filter)
                ]
    ...
    def clear_cache(self, clear_hashes=True):
        try:
            ...
            try:
                dbcon.execute(
                    """DELETE FROM maincache WHERE id=?""", ("ed_user_cloud",)
                )
                dbcon.execute(
                    """DELETE FROM maincache WHERE id LIKE ?""", ("ed_user_cloud%",)
                )
