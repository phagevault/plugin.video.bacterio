    ...
    def resolve_magnet(
        ...
        try:
            ...
            valid_results = [
                i
                for i in links
                if any(i.get("n").lower().endswith(x) for x in extensions)
                and not i.get("l", "") == ""
            ]
            if valid_results:
                if season:
                    correct_files = [
                        ...
                        if seas_ep_filter(season, episode, i["n"])
                    ]
                    if correct_files:
                    ...
                    media_id = max(valid_results, key=lambda x: x.get("s")).get(
                        "l", None
                    )
    ...
    def display_magnet_pack(self, magnet_url, info_hash):
        ...
        try:
            ...
            if not transfer_id:
                return None
            end_results = [
                {"link": i["l"], "filename": i["n"], "size": i["s"]}
                for i in links
                if any(i.get("n").lower().endswith(x) for x in extensions)
                and not i.get("l", "") == ""
            ]
    ...
    def clear_cache(self, clear_hashes=True):
        try:
            ...
            try:
                ...
                dbcon.execute(
                    "DELETE FROM maincache WHERE id LIKE 'ad_list_transfer_%'"
                )
