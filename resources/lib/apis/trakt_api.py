...
def trakt_movies_related(imdb_id):
    ...
    params = {
        "path": "movies/%s/related?extended=full",
        "path_insert": imdb_id,
        "params": {"limit": 20},
    }
...
def trakt_tv_related(imdb_id):
    ...
    params = {
        "path": "shows/%s/related?extended=full",
        "path_insert": imdb_id,
        "params": {"limit": 20},
    }
    return lists_cache_object(get_trakt, string, params)
...
def trakt_tv_certifications(certification, page_no):
    ...
    params = {
        "path": "shows/collected/all%s",
        "params": {"genres": "-anime", "certifications": certification, "limit": 20},
        "page_no": page_no,
    }
    return lists_cache_object(get_trakt, string, params)
...
def trakt_tv_search(query, page_no):
    def _process(dummy_arg):
        return call_trakt(
            "search/show",
            params={"genres": "-anime", "query": query, "limit": 20},
            with_auth=False,
            pagination=True,
            page_no=page_no,
        )
...
def trakt_anime_certifications(certification, page_no):
    ...
    params = {
        "path": "shows/collected/all%s",
        "params": {"certifications": certification, "genres": "anime", "limit": 20},
        "page_no": page_no,
    }
    return lists_cache_object(get_trakt, string, params)
...
def trakt_watched_status_mark(
    action, media, media_id, tvdb_id=0, season=None, episode=None, key="tmdb"
):
    ...
    if not success:
        if media != "movies" and tvdb_id != 0 and key != "tvdb":
            return trakt_watched_status_mark(
                action, media, tvdb_id, 0, season, episode, "tvdb"
            )
    return success
...
def trakt_watchlist(media_type, dummy_arg):
    ...
    if not settings.show_unaired_watchlist():
        ...
        str_format = (
            "%Y-%m-%d" if media_type in ("movie", "movies") else "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        data = [
            i
            for i in data
            if i.get("released", None)
            and js2date(i.get("released"), str_format, remove_time=True) <= current_date
        ]
        ...
        return [
            {
                ...
                "released": i[key].get(r_key)
                if i[key].get(r_key)
                else (
                    "2050-01-01"
                    if media_type == "movie"
                    else "2050-01-01T01:00:00.000Z"
                ),
            }
            for i in data
        ]
...
def hide_unhide_progress_items(params):
    action, media_type, media_id, list_type = (
        params["action"],
        params["media_type"],
        params["media_id"],
        params["section"],
    ...
    url = (
        "users/hidden/%s" % list_type
        if action == "drop"
        else "users/hidden/%s/remove" % list_type
    )
...
def trakt_lists_with_media(media_type, imdb_id):
    def _process(foo):
        ...
        result = [
            ...
            for i in data
            if i["item_count"] > 0
            and i["ids"]["slug"] not in ("", "None", None)
    ...
    params = {
        "path": "%s/%s/lists/personal",
        "path_insert": (media_type, imdb_id),
        "params": {"limit": 100},
        "pagination": False,
    }
    ...
    for c, i in enumerate(data):
        try:
            ...
            if _type in ("movie", "show"):
                ...
                data = {
                    "media_ids": i[_type]["ids"],
                    "title": i[_type]["title"],
                    "type": _type,
                    "order": c,
                    "released": i[_type][r_key],
                    "media_type": _type,
                }
            elif _type == "season":
                data = {
                    "tmdb_id": i["show"]["ids"]["tmdb"],
                    "season": i[_type]["number"],
                    "type": _type,
                    "custom_order": c,
                }
            elif _type == "episode":
    ...
    list_items = [
        {
            ...
            % (
                item["display"],
                " [I](x%02d)[/I]" % item["item_count"] if "item_count" in item else "",
            )
        }
        for item in used_lists
    ]
    kwargs = {
...
def trakt_indicators_movies():
    def _process(item):
        ...
        if not tmdb_id:
            return
        insert_append(
            ("movie", tmdb_id, "", "", item["last_watched_at"], movie["title"])
        )
    ...
    def _process(item):
        ...
        for s in seasons:
            ...
            for e in episodes:
                ...
                if reset_at and reset_at > js2date(
                    last_watched_at, "%Y-%m-%dT%H:%M:%S.%fZ"
                ):
                    continue
                insert_append(
                    ("episode", tmdb_id, season_no, e["number"], last_watched_at, title)
                )

    insert_list = []
    insert_append = insert_list.append
    params = {
        "path": "users/me/watched/shows?extended=full%s",
        "with_auth": True,
        "pagination": False,
    }
    result = get_trakt(params)
...
def trakt_progress_movies(progress_info):
    ...
    insert_append = insert_list.append
    progress_items = [
        i for i in progress_info if i["type"] == "movie" and i["progress"] > 1
    ]
...
def trakt_progress_tv(progress_info):
    ...
    progress_items = [
        i for i in progress_info if i["type"] == "episode" and i["progress"] > 1
    ]
    if not progress_items:
        return
...
def trakt_official_status(media_type):
    ...
    if exclude_http in ("true", ""):
        return True
    media_setting = (
        "scrobble_movie" if media_type in ("movie", "movies") else "scrobble_episode"
    )
    try:
...
def trakt_get_my_calendar(recently_aired, current_date):
    def _process(dummy):
        ...
        data = [
            {
                "sort_title": "%s s%s e%s"
                % (
                    i["show"]["title"],
                    str(i["episode"]["season"]).zfill(2),
                    str(i["episode"]["number"]).zfill(2),
                ),
                "media_ids": i["show"]["ids"],
...
def trakt_calendar_days(recently_aired, current_date):
    if recently_aired:
        ...
        previous_days = int(get_setting("bacterio.trakt.calendar_previous_days", "0"))
        future_days = int(get_setting("bacterio.trakt.calendar_future_days", "7"))
...
def trakt_sync_activities(force_update=False):
    ...
    def clear_properties(media_type):
        for item in ((True, True), (True, False), (False, True), (False, False)):
            kodi_utils.clear_property(
                "1_%s_%s_%s_watched" % (media_type, item[0], item[1])
            )
    ...
    def _check_daily_expiry():
        return int(time.time()) >= int(
            get_setting("bacterio.trakt.next_daily_clear", "0")
        )
    ...
    if _compare(
        latest_movies["collected_at"], cached_movies.get("collected_at", fallback_date)
    ):
        trakt_cache.clear_trakt_collection_watchlist_data("collection", "movie")
    if _compare(
        latest_episodes["collected_at"],
        cached_episodes.get("collected_at", fallback_date),
    ):
        trakt_cache.clear_trakt_collection_watchlist_data("collection", "tvshow")
    if _compare(
        latest_movies["watchlisted_at"],
        cached_movies.get("watchlisted_at", fallback_date),
    ):
        trakt_cache.clear_trakt_collection_watchlist_data("watchlist", "movie")
    if _compare(
        latest_shows["watchlisted_at"],
        cached_shows.get("watchlisted_at", fallback_date),
    ):
        trakt_cache.clear_trakt_collection_watchlist_data("watchlist", "tvshow")
    if _compare(
        latest_shows["dropped_at"], cached_shows.get("dropped_at", fallback_date)
    ):
        clear_properties("episode")
        trakt_cache.clear_trakt_hidden_data("dropped")
    if _compare(
        latest_movies["watched_at"], cached_movies.get("watched_at", fallback_date)
    ):
        clear_properties("movie")
        trakt_indicators_movies()
    if _compare(
        latest_episodes["watched_at"], cached_episodes.get("watched_at", fallback_date)
    ):
        clear_properties("episode")
        trakt_indicators_tv()
        # clear_tvshow_watched_cache = True
    if _compare(
        latest_movies["paused_at"], cached_movies.get("paused_at", fallback_date)
    ):
        refresh_movies_progress = True
    if _compare(
        latest_episodes["paused_at"], cached_episodes.get("paused_at", fallback_date)
    ):
        refresh_shows_progress = True
    if _compare(
        latest_lists["updated_at"], cached_lists.get("updated_at", fallback_date)
    ):
