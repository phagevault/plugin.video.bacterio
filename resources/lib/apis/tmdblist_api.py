...
class TMDbListAPI:
    def __init__(self):
        ...
        self.read_access_token = (
            "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiMzcwYjYwNDQ3NzM3NzYyY2EzODQ1N2JkNzc1NzliMyIsIm5iZiI6MTY1MDIzNTExOS4wOSwic3ViIjoiNjI1Yzk2ZWZiYjI2MDIxMT"
    ...
    def auth(self):
        ...
        headers = {
            ...
            "Authorization": "Bearer %s" % self.read_access_token,
        }
        data = requests.post(
            "%s/auth/request_token" % self.base_url, headers=headers, timeout=20
        ).json()
        if not "success" in data:
            return notification("Failed to Auth Account")
        request_token = data["request_token"]
        token_url = (
            "https://www.themoviedb.org/auth/access?request_token=%s" % request_token
        )
        ...
        progressDialog = progress_dialog(
            heading="TMDb Account Authorization", icon=qr_code
        )
        ...
        while not progressDialog.iscanceled() and count >= 0 and success == None:
            try:
                ...
                response = requests.post(
                    "%s/auth/access_token" % self.base_url,
                ...
                progressDialog.update(
                    "Please Scan the QR Code%s[CR]Confirm Access to your TMDb Account"
                    % p_dialog_insert,
                    count,
                )
                sleep(2500)
    ...
    def revoke(self):
        ...
        data = requests.delete(
            "%s/auth/access_token" % self.base_url,
            json={"access_token": self.read_access_token},
            headers=headers,
            timeout=20,
        ).json()
        if not "success" in data:
    ...
    def get_user_lists(self):
        def _process_multi(page_no):
            try:
                results_extend(
                    self.request_data(url % (self.base_url, account_id, page_no))[
                        "results"
                    ]
                )
            except:
        ...
        def _process(dummy):
            ...
            if total_pages > 1:
                threads = list(
                    make_thread_list(_process_multi, range(2, total_pages + 1))
                )
    ...
    def get_list_details(self, list_id):
        def _process_multi(page_no):
            try:
                results_extend(
                    self.request_data(url % (self.base_url, list_id, page_no))[
                        "results"
                    ]
                )
            except:
    ...
    def item_status(self, list_id, media_type, media_id):
        ...
        return self.request_data(
            url, params={"media_type": media_type, "media_id": int(media_id)}
        )
    ...
    def request_data(self, url, params=None, data=None, method="get"):
        headers = {
            "accept": "application/json",
            "content-type": "application/json",
            "Authorization": "Bearer %s" % get_setting("bacterio.tmdb.token"),
        }
        try:
            result = session.request(
                method, url, params=params, json=data, headers=headers, timeout=90
            ).json()
        except:
            result = None
        return result
