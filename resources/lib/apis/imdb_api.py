...
def imdb_people_id(actor_name):
    ...
    url, url_backup = (
        "https://sg.media-imdb.com/suggests/%s/%s.json"
        % (name[0], name.replace(" ", "%20")),
        "https://www.imdb.com/search/name/?name=%s" % name,
    )
    params = {
        "url": url,
        "action": "imdb_people_id",
        "name": name,
...
def get_imdb(params):
    ...
    if action == "imdb_more_like_this":
        ...
        try:
            ...
            result = result.split("<span>Storyline</span>")[0].split(
        ...
        imdb_list = [
            i for n, i in enumerate(imdb_list) if i not in imdb_list[n + 1 :]
        ]  # remove duplicates
    if action in ("imdb_trivia", "imdb_blunders"):
        ...
        def _process():
            for count, item in enumerate(items, 1):
                try:
                    content = re.sub(
                        r'<a class="ipc-md-link ipc-md-link--entity" href="\S+">',
                        "",
                        item,
                    ).replace("</a>", "")
        ...
        def _process():
            ...
            for item in all_reviews:
                try:
                    try:
                        content = re.findall(
                            r'plaidHtml":"(.*)","__typename":"Markdown', item
                        )[0]
                        try:
                    ...
                    try:
                        title = re.findall(
                            r'"summary":{"originalText":"(.*)","__typename":"ReviewSummary',
                            item,
                        )[0]
                        title = replace_html_codes(
                            title.replace("</a>", "")
                            .replace("<p> ", "")
                            .replace("<br />", "")
                            .replace("  ", "")
                    ...
                    if spoiler == "true":
                        review = (
                            "[B][COLOR red][%s][/COLOR][CR][/B]" % spoiler_str + review
                        )
                    count += 1
                    yield review
                except:
        ...
        if not imdb_list:
            try:
                ...
                result = remove_accents(result.text)
                result = result.replace("\n", " ")
                result = parseDOM(result, "div", attrs={"class": "lister-item-image"})[
                    0
                ]
        ...
        results = parseDOM(
            result,
            "section",
            attrs={"class": "ipc-page-section ipc-page-section--base"},
        )
        for item in results:
            if "contentRating" in item:
                continue
            if "Certifications" in item:
                continue
            item_dict = {}
            try:
                title_data = re.search(
                    r'<span id="(.+?)">(.+?)</span>', item, re.DOTALL
                ).group(0)
                title = replace_html_codes(
            ...
            try:
                ranking = replace_html_codes(
                    re.search(
                        r'<div class="ipc-signpost__text" role="presentation">(.+?)</div>',
                        item,
                        re.DOTALL,
                    ).group(1)
                )
            ...
            try:
                listings = re.findall(
                    r'<div class="ipc-html-content-inner-div" role="presentation">(.+?)</div>',
                    item,
                )
            ...
            if listings:
                item_dict["content"] = "\n\n".join(
                    ["%02d. %s" % (count, i) for count, i in enumerate(listings, 1)]
                )
            elif item_dict["ranking"] == "none":
                continue
            item_dict["total_count"] = len(listings)
            if item_dict:
                imdb_append(item_dict)
...
def clear_imdb_cache(silent=False):
    try:
        ...
        results = dbcon.execute(
            "SELECT id FROM maincache WHERE id LIKE ?", ("imdb_%",)
        ).fetchall()
...
def refresh_imdb_meta_data(imdb_id):
    try:
        ...
        for item in (insert1, insert2):
            results = dbcon.execute(
                "SELECT id FROM maincache WHERE id LIKE ?", (item,)
            ).fetchall()
