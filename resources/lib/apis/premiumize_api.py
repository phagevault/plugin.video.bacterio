...
class PremiumizeAPI:
    ...
    def auth(self):
        ...
        copy2clip(auth_url)
        content = (
            "Authorize Debrid Services[CR]Navigate to: [B]%s[/B][CR]Enter the following code: [B]%s[/B]"
            % (auth_url, user_code)
        )
    ...
    def unrestrict_link(self, link):
        ...
        response = self._post(url, data)
        try:
            return self.add_headers_to_url(response["content"][0]["link"])
        except:
            return None
    ...
    def resolve_magnet(
        ...
        try:
            ...
            valid_results = [
                i
                for i in content
                if any(i.get("path").lower().endswith(x) for x in extensions)
                and not i.get("link", "") == ""
            ]
            ...
            if season:
                ...
                file_url = max(valid_results, key=lambda x: int(x.get("size"))).get(
                    "link", None
                )
            ...
            if file_url:
                if store_to_cloud:
                    Thread(target=self.create_transfer, args=(magnet_url,)).start()
    ...
    def display_magnet_pack(self, magnet_url, info_hash):
        ...
        try:
            ...
            for item in result.get("content"):
                if (
                    any(item.get("path").lower().endswith(x) for x in extensions)
                    ...
                    try:
                        ...
                        path = item["path"]
                    append(
                        {"link": item["link"], "filename": path, "size": item["size"]}
                    )
            return end_results
        except:
            return None
    ...
    def _post(self, url, data={}):
        if self.token in ("empty_setting", "") and not "token" in url:
            return None
        if not "token" in url:
            url = "https://www.premiumize.me/api/" + url
        response = requests.post(
            url, data=data, headers=self.headers(), timeout=20
        ).text
        try:
            return json.loads(response)
        except:
    ...
    def clear_cache(self, clear_hashes=True):
        try:
            ...
            try:
                try:
                    user_cloud_cache = dbcon.execute(
                        """SELECT id FROM maincache WHERE id LIKE ?""",
                        ("pm_user_cloud%",),
                    ).fetchall()
            ...
            try:
                dbcon.execute(
                    """DELETE FROM maincache WHERE id=?""", ("pm_transfers_list",)
                )
