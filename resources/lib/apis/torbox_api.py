...
class TorBoxAPI:
    def __init__(self):
        self.token = get_setting("bacterio.tb.token")
    ...
    def _post(self, url, params=None, json=None, data=None):
        ...
        response = session.post(
            url, params=params, json=json, data=data, headers=headers, timeout=20
        )
        return response.json()
    ...
    def unrestrict_link(self, file_id):
        ...
        try:
            return self._get("torrents/requestdl", data=data)["data"]
        except:
            return None
    ...
    def unrestrict_usenet(self, file_id):
        ...
        try:
            return self._get("usenet/requestdl", params=params)["data"]
        except:
            return None
    ...
    def resolve_magnet(
        ...
        try:
            ...
            extras_filtering_list = tuple(
                i for i in extras_filter if not i in title.lower()
            )
            ...
            if season:
                selected_files = [
                    ...
                    if seas_ep_filter(season, episode, i["filename"])
                ]
            else:
                ...
                selected_files = [
                    i
                    for i in selected_files
                    if not any(x in i["filename"] for x in extras_filtering_list)
                ]
    ...
    def clear_cache(self, clear_hashes=True):
        try:
            ...
            try:
                dbcon.execute(
                    """DELETE FROM maincache WHERE id=?""", ("tb_user_cloud",)
                )
                dbcon.execute(
                    """DELETE FROM maincache WHERE id LIKE ?""", ("tb_user_cloud%",)
                )
            ...
            return False
        if False in (user_cloud_success, hash_cache_status_success):
            return False
        return True
