...
class source:
    def __init__(
        ...
        self.processed_internal_scrapers_append = (
            self.processed_internal_scrapers.append
        )
        self.internal_scrapers, self.prescrape_sources = (
            internal_scrapers,
            prescrape_sources,
        )
        self.internal_activated, self.internal_prescraped = (
            len(self.internal_scrapers) > 0,
            len(self.prescrape_sources) > 0,
        )
        self.processed_prescrape, self.threads_completed = False, False
        self.timeout = (
            60
            if disabled_ext_ignored
            else int(get_setting("bacterio.results.timeout", "20"))
        )
        self.sources_total = self.sources_4k = self.sources_1080p = (
            self.sources_720p
        ) = self.sources_sd = 0
        self.final_total = self.final_4k = self.final_1080p = self.final_720p = (
            self.final_sd
        ) = 0
        self.count_tuple = (
            ...
            ("sources_total", "", self.quality_length_final),
        )
        self.count_tuple_final = (
            ("final_4k", "4K", self._quality_length),
            ("final_1080p", "1080p", self._quality_length),
            ("final_720p", "720p", self._quality_length),
            ("final_sd", "", self._quality_length_sd),
    ...
    def results(self, info):
        if not self.source_dict:
            return
        try:
            self.media_type, self.tmdb_id, self.orig_title = (
                info["media_type"],
                str(info["tmdb_id"]),
                info["title"],
            )
            self.season, self.episode, self.total_seasons = (
                info["season"],
                info["episode"],
                info["total_seasons"],
            )
            ...
            self.single_expiry, self.season_expiry, self.show_expiry = info[
                "expiry_times"
            ]
    ...
    def get_sources(self):
        def _scraperDialog():
            ...
            while (
                not self.progress_dialog.iscanceled()
                ...
                try:
                    ...
                    percent = (
                        max((time.time() - start_time), 0) / float(self.timeout)
                    ) * 100
                    ...
                    if self.threads_completed:
                        ...
                        if len_alive_threads == 0 or percent >= 100:
                            break
                    elif percent >= 100:
                        break
        ...
        def _background():
            ...
            while time.time() < end_time:
                ...
                if len_alive_threads <= 5:
                    return
                if len(self.sources) >= 100 * len_alive_threads:
                    return
        ...
        if self.media_type == "movie":
            ...
            self.season_packs, self.show_packs = source_utils.pack_enable_check(
                self.meta, self.season, self.episode
            )
            if self.season_packs:
                ...
                if pack_capable:
                    self.source_dict.extend(
                        [(i[0], i[1], "Season") for i in pack_capable]
                    )
                    if self.show_packs:
                        self.source_dict.extend(
                            [(i[0], i[1], "Show") for i in pack_capable]
                        )
        ...
        if current_results:
            return self.process_results(current_results)
        return []

    def process_movie_threads(self):
        for i in self.source_dict:
            provider, module = i[0], i[1]
            threaded_object = Thread(
                target=self.get_movie_source, args=(provider, module), name=provider
            )
    ...
    def process_episode_threads(self):
        for i in self.source_dict:
            ...
            threaded_object = Thread(
                target=self.get_episode_source,
                args=(provider, module, pack_arg),
    ...
    def get_movie_source(self, provider, module):
        sources = external_cache.get(
            provider, self.media_type, self.tmdb_id, self.title, self.year, "", ""
        )
        if sources == None:
    ...
    def get_episode_source(self, provider, module, pack):
        if pack in ("Season", "Show"):
            ...
            s_check, e_check = self.season, self.episode
        sources = external_cache.get(
            provider,
            self.media_type,
            self.tmdb_id,
        ...
        if sources:
            if pack == "Season":
                sources = [
                    i
                    for i in sources
                    if not "episode_start" in i
                    or i["episode_start"] <= self.episode <= i["episode_end"]
                ]
            elif pack == "Show":
                sources = [i for i in sources if i["last_season"] >= self.season]
            if not self.background:
                self.process_quality_count(sources)
            self.sources.extend(sources)
        del module
    ...
    def process_results(self, results):
        ...
        def _process_cache_check(provider, function):
            ...
            if not self.background:
                self.process_quality_count_final(
                    [i for i in results if i["hash"] in cached]
                )
        ...
        try:
            if not self.background and self.all_internal_sources:
                self.process_quality_count_final(self.all_internal_sources)
            final_results = []
            ...
            debrid_check_threads = [
                Thread(
                    target=_process_cache_check,
                    args=self.debrid_runners[item],
                    name=item,
                )
                for item in self.active_debrid
            ]
            [i.start() for i in debrid_check_threads]
            if self.background:
                [i.join() for i in debrid_check_threads]
            else:
        ...
        try:
            for i in sources:
                try:
                    ...
                    try:
                        size = i_get("size")
                        if "package" in i and provider not in (
                            "torrentio",
                            "knightcrawler",
                            "selfhosted",
                        ):
                            if i_get("package") == "season":
                                divider = self.season_divider
                            else:
    ...
    def process_quality_count_final(self, sources):
        for item in self.count_tuple_final:
            setattr(self, item[0], getattr(self, item[0]) + item[2](sources, item[1]))
