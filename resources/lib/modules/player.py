...
class BacterioPlayer(xbmc.Player):
    def __init__(self):
    ...
    def check_playback_start(self):
        ...
        while self.playback_successful is None:
            ...
            if not self.sources_object.progress_dialog:
                ...
                self.sources_object.progress_dialog.iscanceled()
                or self.kodi_monitor.abortRequested()
            ):
    ...
    def monitor(self):
        try:
            ...
            if self.media_type == "episode":
                ...
                disable_autoplay_next_episode = (
                    self.sources_object.disable_autoplay_next_episode
                )
                if disable_autoplay_next_episode:
                    ku.notification(
                        "Scrape with Custom Values - Autoplay Next Episode Cancelled",
                        4500,
                    )
                if any(
                    (play_random_continual, play_random, disable_autoplay_next_episode)
                ):
                    self.autoplay_nextep, self.autoscrape_nextep = False, False
                else:
                    self.autoplay_nextep, self.autoscrape_nextep = (
                        self.sources_object.autoplay_nextep,
                        self.sources_object.autoscrape_nextep,
                    )
            else:
                show_stinger, stinger_use_chapters, stingers_percentage_fallback = (
                    st.stingers_show(),
                    st.stingers_use_chapters(),
                    st.stingers_percentage(),
                )
                play_random_continual, self.autoplay_nextep, self.autoscrape_nextep = (
                    False,
                    False,
                    False,
                )
            while total_check_time <= 30 and not ku.get_visibility(
                    ...
                    if self.media_type == "episode":
                        if self.autoplay_nextep or self.autoscrape_nextep:
                            if not self.nextep_info_gathered:
                                self.info_next_ep()
                            if (
                                round(self.total_time - self.curr_time)
                                <= self.start_prep
                            ):
                                self.run_next_ep()
                                break
                        ...
                        final_chapter = (
                            (self.final_chapter(75) or stingers_percentage_fallback)
                            if stinger_use_chapters
                            else stingers_percentage_fallback
                        )
    ...
    def media_watched_marker(self, force_watched=False):
        ...
        try:
            if self.current_point >= 90 or force_watched:
                watched_function = (
                    ws.mark_movie if self.media_type == "movie" else ws.mark_episode
                )
                watched_params = {
                ...
                Thread(
                    ...
                    args=(watched_function, watched_params),
                ).start()
            else:
                ku.clear_property("bacterio.random_episode_history")
    ...
    def run_movie_stingers(self):
        ...
        if stinger_keys:
            ...
            Thread(
                target=lambda: open_window(
                    ("windows.playback_notifications", "StingersNotification"),
    ...
    def set_resume_point(self, listitem):
        if self.playback_percent > 0.0:
            listitem.setProperty("StartPercent", str(self.playback_percent))

    def info_next_ep(self):
        self.nextep_info_gathered = True
        try:
            play_type = (
                "autoplay_nextep" if self.autoplay_nextep else "autoscrape_nextep"
            )
            nextep_settings = st.auto_nextep_settings(play_type)
            watching_check = nextep_settings["watching_check"]
            still_watching_check = (
                15 if self.meta_get("watch_count") == watching_check else 0
            )
            final_chapter = (
                self.final_chapter(90) if nextep_settings["use_chapters"] else None
            )
            percentage = (
                100 - final_chapter
                if final_chapter
                else nextep_settings["window_percentage"]
            )
            window_time = (
                round((percentage / 100) * self.total_time) + still_watching_check
            )
    ...
    def set_constants(self, url, obj):
        ...
        if not self.is_generic:
            ...
            self.meta_get, self.kodi_monitor, self.playback_percent = (
                self.meta.get,
                ku.kodi_monitor(),
                self.sources_object.playback_percent or 0.0,
            )
            self.playing_filename = self.sources_object.playing_filename
            self.media_marked, self.nextep_info_gathered, self.movie_stingers_run = (
                False,
                False,
                False,
            )
    ...
    def set_playback_properties(self):
        try:
            trakt_ids = {
                "tmdb": self.tmdb_id,
                "imdb": self.imdb_id,
                "slug": make_trakt_slug(self.title),
            }
            if self.media_type == "episode":
    ...
    def clear_playback_properties(self):
        ku.clear_property("bacterio.window_stack")
