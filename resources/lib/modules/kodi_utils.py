...
def random_valid_type_check():
    return {
        "build_movie_list": "movie",
        "build_tvshow_list": "tvshow",
        "build_season_list": "season",
        "build_episode_list": "episode",
        "build_in_progress_episode": "single_episode",
        "build_recently_watched_episode": "single_episode",
        "build_next_episode": "single_episode",
        "build_my_calendar": "single_episode",
        "build_trakt_lists": "trakt_list",
        "trakt.list.build_trakt_list": "trakt_list",
        "build_trakt_lists_contents": "trakt_list",
        "personal_lists.build_personal_list": "personal_list",
        "build_personal_lists_contents": "personal_list",
        "tmdblist.build_tmdb_list": "tmdb_list",
        "build_tmdb_lists_contents": "tmdb_list",
    }
...
def random_episodes_check():
    return {
        ...
        "build_my_calendar": "episode.trakt",
    }
...
def extras_button_label_values():
    return {
        "movie": {
            "movies_play": "Play",
            "show_trailers": "Trailer",
            "show_images": "Images",
            "show_extrainfo": "Extra Info",
            "show_genres": "Genres",
            "show_director": "Director",
            "show_options": "Options",
            "show_recommended": "Recommended",
            "show_more_like_this": "More Like This",
            "show_trakt_manager": "Trakt Lists",
            "show_personallists_manager": "Personal Lists",
            "show_tmdb_manager": "TMDb Lists",
            "show_favorites_manager": "Favorites Lists",
            "playback_choice": "Play Options",
            "show_plot": "Plot",
            "show_keywords": "Keywords",
            "show_in_trakt_lists": "In Trakt Lists",
            "close_all": "Close",
        },
        "tvshow": {
            "tvshow_browse": "Browse",
            "show_trailers": "Trailer",
            "show_images": "Images",
            "show_extrainfo": "Extra Info",
            "show_genres": "Genres",
            "play_nextep": "Play Next",
            "show_options": "Options",
            "show_recommended": "Recommended",
            "show_more_like_this": "More Like This",
            "show_trakt_manager": "Trakt Lists",
            ...
            "show_tmdb_manager": "TMDb Lists",
            "show_favorites_manager": "Favorites Lists",
            "play_random_episode": "Play Random",
            "show_plot": "Plot",
            "show_keywords": "Keywords",
            "show_in_trakt_lists": "In Trakt Lists",
            "close_all": "Close",
...
def context_menu_items():
    return {
        "extras": "Extras",
        "options": "Options",
        "playback_options": "Play Options",
        "browse_movie_set": "Browse Movie Set",
        "browse_seasons": "Browse TV Seasons",
        "browse_episodes": "Browse Season Episodes",
        "recommended": "Browse Recommended",
        "related": "Browse Related",
        "more_like_this": "Browse More Like This",
        "similar": "Browse Similar",
        "in_trakt_list": "In Trakt Lists",
        "trakt_manager": "Trakt Lists Manager",
        "personal_manager": "Personal Lists Manager",
        "tmdb_manager": "TMDb Lists Manager",
        "favorites_manager": "Favorites Manager",
        "mark_watched": "Mark Watched/Unwatched",
        "unmark_previous_episode": "Unmark Previous Watched Episode",
        "exit": "Exit List",
...
def addon_profile():
    return get_property("bacterio.addon_profile") or translate_path(
        addon_info("profile")
    )
...
def addon_icon_mini():
    return get_property("bacterio.addon_icon_mini") or os.path.join(
        addon_info("path"),
        "resources",
        "media",
        "addon_icons",
...
def get_video_database_path():
    return translate_path(
        "special://profile/Database/MyVideos%s.db"
        % {19: "119", 20: "121", 21: "124"}[kodi_version()]
    )
...
def jsonrpc_get_addons(_type, properties=["thumbnail", "name"]):
    command = {
        ...
        "params": {"type": _type, "properties": properties},
        "id": "1",
    }
...
def progress_dialog(heading="", icon=None):
    ...
    progress_dialog = create_window(
        ("windows.progress", "Progress"),
        "progress.xml",
        heading=heading,
...
def select_dialog(function_list, **kwargs):
    ...
    selection = open_window(
        ("windows.default_dialogs", "Select"), "select.xml", **kwargs
    )
    ...
    if kodi_log:
        ...
        if confirm:
            text = [
                i
                for i in text
                if any(x in i.lower() for x in ("exception", "error", "[test]"))
            ]
    text = "".join(text)
    return open_window(
        ("windows.textviewer", "TextViewer"),
        "textviewer.xml",
        heading=heading,
        text=text,
        font_size=font_size,
...
def player_check(mode, params):
    ...
    if mode == "playback.%s" % playback_key():
        ...
        ok_dialog(
            "External Playback Detected",
            "Playback through external addons is not supported",
        )
...
def volume_checker():
    ...
    try:
        if get_property(
            "bacterio.playback.volumecheck_enabled"
        ) == "false" or get_visibility("Player.Muted"):
            return
        from modules.utils import string_alphanum_to_num

        max_volume = min(
            int(get_property("bacterio.playback.volumecheck_percent") or "50"), 100
        )
        if (
            int(
                100
                - (
                    float(
                        string_alphanum_to_num(
                            get_infolabel("Player.Volume").split(".")[0]
                        )
                    )
                    / 60
                )
...
def get_all_icons():
    ...
    def _process(dummy):
        try:
            results = requests.get(
                "https://api.github.com/repos/%s/%s/contents/packages/media/icons"
                % (username, location)
            )
    ...
    username, location = (
        get_property("bacterio.update.username"),
        get_property("bacterio.update.location"),
    )
...
def get_all_addon_icons():
    ...
    def _process(dummy):
        try:
            results = requests.get(
                "https://api.github.com/repos/%s/%s/contents/packages/addon_icons"
                % (username, location)
            )
            return results
        except:
...
def upload_logfile(params):
    ...
    log_files = [
        ("Current Kodi Log", "kodi.log"),
        ("Previous Kodi Log", "kodi.old.log"),
    ]
    list_items = [{"line1": i[0]} for i in log_files]
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Choose Which Log File to Upload",
        "narrow_window": "true",
    }
            ...
            while not progressDialog.iscanceled() and count >= 0 and success == None:
                try:
                    ...
                    progressDialog.update(
                        "Share or Access with this url: [B]%s[/B][CR]Or Access using this QR Code"
                        % url,
                        count,
                    )
                    sleep(2500)
...
def fetch_kodi_imagecache(image):
    ...
    try:
        dbcon = database.connect(
            translate_path("special://database/Textures13.db"), timeout=40.0
        )
        dbcur = dbcon.cursor()
