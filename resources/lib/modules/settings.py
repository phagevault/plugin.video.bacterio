...
_CONTEXT_MENU_DEFAULT_ORDER = (
    "extras,options,playback_options,browse_movie_set,browse_seasons,browse_episodes,"
    "recommended,related,more_like_this,similar,in_trakt_list,trakt_manager,"
    "personal_manager,tmdb_manager,favorites_manager,mark_watched,"
    "unmark_previous_episode,exit,refresh,reload"
)
...
def trakt_user_active():
    return get_setting("bacterio.trakt.user", "empty_setting") not in (
        None,
        "empty_setting",
        "",
    )
...
def tmdblist_user_active():
    return get_setting("bacterio.tmdb.account_id", "empty_setting") not in (
        None,
        "empty_setting",
        "",
    )
...
def enabled_debrids_check(debrid_service):
    if not get_setting("bacterio.%s.enabled" % debrid_service) == "true":
        return False
    return authorized_debrid_check(debrid_service)


def authorized_debrid_check(debrid_service):
    if get_setting("bacterio.%s.token" % debrid_service) in (None, "", "empty_setting"):
        return False
    return True
...
def playback_settings():
    return (
        int(get_setting("bacterio.playback.watched_percent", "90")),
        int(get_setting("bacterio.playback.resume_percent", "5")),
    )
...
def download_directory(media_type):
    download_directories_dict = {
        "movie": "bacterio.movie_download_directory",
        "episode": "bacterio.tvshow_download_directory",
        "thumb_url": "bacterio.image_download_directory",
        "image_url": "bacterio.image_download_directory",
        "image": "bacterio.image_download_directory",
        "premium": "bacterio.premium_download_directory",
        None: "bacterio.premium_download_directory",
        "None": False,
    }
...
def ai_model_order():
    return get_setting(
        "bacterio.ai_model.order",
        "gemini-2.5-flash-lite,llama-3.3-70b-versatile,gemma-3-27b-it,llama-3.1-8b-instant",
    ).split(",")
...
def source_folders_directory(media_type, source):
    setting = (
        "bacterio.%s.movies_directory" % source
        if media_type == "movie"
        else "bacterio.%s.tv_shows_directory" % source
    )
...
def page_limit(is_home):
    return int(
        get_setting(
            {
                True: "bacterio.paginate.limit_widgets",
                False: "bacterio.paginate.limit_addon",
            }[is_home],
            "20",
        )
    )
...
def autoplay_next_episode():
    if (
        auto_play("episode")
        and get_setting("bacterio.autoplay_next_episode", "false") == "true"
    ):
        return True
    else:
        return False
...
def autoscrape_next_episode():
    if (
        not auto_play("episode")
        and get_setting("bacterio.autoscrape_next_episode", "false") == "true"
    ):
        return True
    else:
        return False
...
def auto_nextep_settings(play_type):
    ...
    window_percentage = 100 - int(
        get_setting("bacterio.%s_next_window_percentage" % play_type, "95")
    )
    ...
    if play_type == "autoplay":
        ...
        default_action = {"0": "play", "1": "cancel", "2": "pause"}[
            get_setting("bacterio.autoplay_default_action", "1")
        ]
    else:
...
def personal_lists_unseen_highlight():
    if get_setting("bacterio.personal_list.highlight_unseen", "false") == "false":
        return None
...
def single_ep_display_format(is_external):
    if is_external:
        setting, default = "bacterio.single_ep_display_widget", "1"
    else:
        setting, default = "bacterio.single_ep_display", ""
...
def extras_enabled_menus():
    setting = get_setting(
        "bacterio.extras.enabled",
        "2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062",
    )
...
def check_prescrape_sources(scraper, media_type):
    ...
    if get_setting("bacterio.check.%s" % scraper) == "true" and auto_play(media_type):
        return True
    else:
        return False
...
def results_sort_order():
    sort_direction = (
        -1 if get_setting("bacterio.results.size_sort_direction") == "0" else 1
    )
    return (
        lambda k: (
            k["quality_rank"],
            k["provider_rank"],
            sort_direction * k["size_rank"],
        ),  # Quality, Provider, Size
        lambda k: (
            k["quality_rank"],
            sort_direction * k["size_rank"],
            k["provider_rank"],
...
def active_internal_scrapers():
    ...
    active = [
        i.split(".")[1] for i in settings if get_setting("bacterio.%s" % i) == "true"
    ]
    return active
...
def auto_resume(media_type, autoplay_status):
    return {0: False, 1: True, 2: autoplay_status}[
        int(get_setting("bacterio.auto_resume_%s" % media_type))
    ]
...
def max_threads():
    if not get_setting("bacterio.limit_concurrent_threads", "false") == "true":
        return 60
...
def nextep_sort_key():
    return {0: "last_played", 1: "first_aired", 2: "name"}[
        int(get_setting("bacterio.nextep.sort_type", "0"))
    ]
...
def rpdb_api_key(media_type):
    if (
        int(get_setting("bacterio.rpdb_enabled", "0"))
        not in {"movie": (1, 3), "tvshow": (2, 3)}[media_type]
    ):
        return None
