...
class Sources():
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('bacterio.results.%s_size_min' % self.media_type, '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = string_to_float(get_setting('bacterio.results.%s_size_max' % self.media_type, '10000'), '10000') / 1000
	...
	def special_filter(self, results, file_type):
		...
		if key == 'HEVC' and enable_setting == 0:
			hevc_max_quality = self._get_quality_rank(get_setting('bacterio.filter.hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('bacterio.%s.display_name' % i), i, settings.source_folders_directory(self.media_type, i))
	...
	def _clear_properties(self):
		...
		for item in def_internal: kodi_utils.clear_property('bacterio.internal_results.%s' % item)
		if self.active_folders:
			for item in self.folder_info: kodi_utils.clear_property('bacterio.internal_results.%s' % item[0])
