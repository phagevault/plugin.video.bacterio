...
from modules import debrid, kodi_utils, metadata, settings, watched_status
from modules.player import BacterioPlayer
from modules.source_utils import (
    get_cache_expiry,
...
class Sources:
    def __init__(self):
        ...
        (
            self.rescrape_cache_ignored,
            self.original_year_ignored,
            self.rescrape_with_all,
            self.rescrape_with_episode_group,
        ) = False, False, False, False
        (
            self.clear_properties,
            self.filters_ignored,
            self.active_folders,
            self.resolve_dialog_made,
            self.episode_group_used,
        ...
        self.default_internal_scrapers = (
            "easynews",
            "rd_cloud",
            "pm_cloud",
            "ad_cloud",
    ...
    def playback_prep(self, params=None):
        ...
        self.random, self.random_continual = (
            params_get("random", "false") == "true",
            params_get("random_continual", "false") == "true",
        )
        ...
        if self.play_type:
            ...
            self.autoplay_nextep, self.autoscrape_nextep = (
                settings.autoplay_next_episode(),
                settings.autoscrape_next_episode(),
            )
        self.autoscrape = self.autoscrape_nextep and self.background
        self.auto_rescrape_cache_ignored, self.auto_rescrape_imdb_year = (
            settings.auto_rescrape_cache_ignored(),
            settings.auto_rescrape_imdb_year(),
        )
        self.auto_rescrape_with_all, self.auto_episode_group = (
            settings.auto_rescrape_with_all(),
            settings.auto_episode_group(),
        )
        self.ignore_scrape_filters = (
            params_get("ignore_scrape_filters", "false") == "true"
        )
        ...
        self.disabled_ext_ignored = (
            params_get("disabled_ext_ignored", self.disabled_ext_ignored) == "true"
        )
        ...
        self.custom_title, self.custom_year = (
            params_get("custom_title", None),
            params_get("custom_year", None),
        )
        self.episode_group_label, self.episode_id = (
            params_get("episode_group_label", ""),
            params_get("episode_id", None),
        )
        self.playcount, self.watch_count = (
            params_get("playcount", None),
            params_get("watch_count", 1),
        )
        if self.media_type == "episode":
            self.season, self.episode = (
                int(params_get("season")),
                int(params_get("episode")),
            )
            self.custom_season, self.custom_episode = (
                params_get("custom_season", None),
                params_get("custom_episode", None),
            )
        ...
        self.sleep_time, self.provider_sort_ranks, self.scraper_settings = (
            100,
            settings.provider_sort_ranks(),
            settings.scraping_settings(),
        )
        self.include_prerelease_results, self.ignore_results_filter = (
            settings.include_prerelease_results(),
            settings.ignore_results_filter(),
        )
        self.limit_resolve = settings.limit_resolve()
        self.weight_size = settings.size_sort_weighted()
        self.sort_function, self.quality_filter = (
            settings.results_sort_order(),
    ...
    def check_episode_group(self):
        try:
            if (
                any([self.custom_season, self.custom_episode])
                or "skip_episode_group_check" in self.params
            ):
                return
            group_info = episode_groups_cache.get(self.tmdb_id)
            if not group_info:
                return
            group_details = metadata.group_episode_data(
                metadata.group_details(group_info["id"]),
                self.episode_id,
                self.season,
                self.episode,
            )
            if group_details:
                self.custom_season, self.custom_episode, self.episode_group_used = (
                    group_details["season"],
                    group_details["episode"],
                    True,
                )
                self.episode_group_label = "[B]CUSTOM GROUP: S%02dE%02d[/B]" % (
                    self.custom_season,
                    self.custom_episode,
                )
        except:
    ...
    def determine_scrapers_status(self):
        ...
        if (
            "external" not in self.active_internal_scrapers
            and self.disabled_ext_ignored
        ):
        ...
        if self.active_external:
            ...
            if not self.debrid_enabled:
                return self.disable_external("No Debrid Services Enabled")
    ...
    def get_sources(self):
        ...
        results = []
        if self.prescrape and any(
            x in self.active_internal_scrapers for x in self.default_internal_scrapers
        ):
    ...
    def collect_results(self):
        ...
        if self.providers:
            for i in self.providers:
                threads_append(
                    Thread(
                        target=self.activate_providers,
                        args=(i[0], i[1], False),
                        name=i[2],
                    )
                )
    ...
    def collect_prescrape_results(self):
        ...
        for i in self.prescrape_scrapers:
            threads_append(
                Thread(
                    target=self.activate_providers, args=(i[0], i[1], True), name=i[2]
                )
            )
    ...
    def process_results(self, results):
        ...
        self.uncached_results = [
            i for i in results if "Uncached" in i.get("cache_provider", "")
        ]
        results = [i for i in results if i not in self.uncached_results]
        ...
        if self.prescrape:
            ...
            autoplay_results = [
                i
                for i in results
                if i["scrape_provider"] in self.active_internal_scrapers
                and settings.autoplay_prescrape(i["scrape_provider"])
            ]
            if autoplay_results:
                self.autoplay = True
                results = autoplay_results
        else:
            self.all_scrapers = list(
                set(self.active_internal_scrapers + self.remove_scrapers)
            )
    ...
    def filter_results(self, results):
        ...
        if self.filter_size_method:
            min_size = (
                string_to_float(
                    get_setting("bacterio.results.%s_size_min" % self.media_type, "0"),
                    "0",
                )
                / 1000
            )
            ...
            if self.filter_size_method == 1:
                duration = self.meta["duration"] or (
                    5400 if self.media_type == "movie" else 2400
                )
                max_size = (
                    (
                        0.125
                        * (
                            0.90
                            * string_to_float(
                                get_setting("results.line_speed", "25"), "25"
                            )
                        )
                    )
                    * duration
                ...
                max_size = (
                    string_to_float(
                        get_setting(
                            "bacterio.results.%s_size_max" % self.media_type, "10000"
                        ),
                        "10000",
                    )
                    / 1000
            ...
            results = [
                i
                for i in results
                if i["scrape_provider"] == "folders"
                or min_size <= i["size"] <= max_size
            ]
            ...
            if key in ("D/VISION", "HDR"):
                if (
                    ...
                    results = [
                        ...
                        for i in results
                        if not (
                            key in i["extraInfo"] and "HYBRID" not in i["extraInfo"]
                        )
                    ]
            else:
                results = [i for i in results if key not in i["extraInfo"]]
    ...
    def sort_preferred_filters(self, results):
        if settings.sort_to_top_filter(self.autoplay):
            try:
                ...
                preference_results = [
                    i for i in results if any(x in i["extraInfo"] for x in preferences)
                ]
    ...
    def sort_first(self, results):
        try:
            ...
            if not sort_first_scrapers:
                return results
            sort_first = [
                i for i in results if i["scrape_provider"] in sort_first_scrapers
            ]
            sort_first.sort(
                key=lambda k: (
                    self._sort_folder_to_top(k["scrape_provider"]),
                    k["quality_rank"],
                )
            )
    ...
    def limit_quality_numbers(self, results):
        ...
        if not quality_limit:
            return results
        quality_counter_dict, limit_list = (
            {"4K": 0, "1080p": 0, "720p": 0, "SD": 0, "SCR": 0, "CAM": 0, "TELE": 0},
            [],
        )
        for i in results:
    ...
    def prepare_internal_scrapers(self):
        ...
        active_internal_scrapers = [
            i for i in self.active_internal_scrapers if i not in self.remove_scrapers
        ]
        if (
            self.prescrape
            and not self.active_external
            and all(
                [
                    settings.check_prescrape_sources(i, self.media_type)
        ...
        if "folders" in active_internal_scrapers:
            ...
            self.folder_info = [
                ...
                if settings.source_folders_directory(self.media_type, i[1])
            ]
            if self.folder_info:
                self.active_folders = True
                self.internal_scraper_names = [
                    i for i in active_internal_scrapers if not i == "folders"
                ] + [i[0] for i in self.folder_info]
            else:
                self.internal_scraper_names = [
                    i for i in active_internal_scrapers if not i == "folders"
                ]
    ...
    def internal_sources(self, prescrape=False):
        ...
        try:
            sourceDict = [
                ...
                if not (
                    prescrape
                    and not settings.check_prescrape_sources(i, self.media_type)
                )
            ]
        except:
            sourceDict = []
    ...
    def external_sources(self):
        ...
        try:
            sourceDict = self.filter_external_sources(
                manual_function_import(self.ext_name, "sources")(
                    specified_folders=["torrents"], ret_all=self.disabled_ext_ignored
                )
            )
        except:
            sourceDict = []
    ...
    def filter_external_sources(self, sourceDict):
        if settings.external_filter_sources():
            if any([self.disabled_ext_ignored, len(sourceDict) <= 5]):
                return sourceDict
    ...
    def folder_sources(self):
        def import_info():
            for item in self.folder_info:
                ...
                yield (
                    "folders",
                    (module, (item[1], scraper_name, item[2])),
                    scraper_name,
                )
    ...
    def get_folderscraper_info(self):
        folder_info = [
            (
                get_setting("bacterio.%s.display_name" % i),
                i,
                settings.source_folders_directory(self.media_type, i),
            )
    ...
    def scrapers_dialog(self):
        def _scraperDialog():
            ...
            while (
                not self.progress_dialog.iscanceled() and not monitor.abortRequested()
            ):
                try:
                    remaining_providers = [
    ...
    def _process_post_results(self):
        if (
            self.auto_rescrape_cache_ignored in (1, 2)
            ...
            and not self.rescrape_cache_ignored
        ):
            self.rescrape_cache_ignored = True
            if self.auto_rescrape_cache_ignored == 1 or kodi_utils.confirm_dialog(
                heading=self.meta.get("rootname", ""),
                text="No results.[CR]Retry With Cache Check Disabled?",
            ):
                self.threads, self.prescrape, self.external_cache_check = (
                    [],
                    False,
                    False,
                )
                return self.get_sources()
        if (
            self.auto_rescrape_imdb_year in (1, 2)
            and self.active_external
            and not self.orig_results
            and not self.original_year_ignored
            and not self.meta.get("custom_year")
        ):
            self.original_year_ignored = True
            if self.auto_rescrape_imdb_year == 1 or kodi_utils.confirm_dialog(
                heading=self.meta.get("rootname", ""),
                text="No results.[CR]Retry With IMDb Year Data?",
            ):
                ...
                if imdb_year != self.get_search_year():
                    ...
                    return self.get_sources()
        if (
            self.auto_rescrape_with_all in (1, 2)
            and self.active_external
            and not self.rescrape_with_all
        ):
            self.rescrape_with_all = True
            if self.auto_rescrape_with_all == 1 or kodi_utils.confirm_dialog(
                heading=self.meta.get("rootname", ""),
                text="No results.[CR]Retry With All Scrapers?",
            ):
                self.threads, self.disabled_ext_ignored, self.prescrape = (
                    [],
                    True,
                    False,
                )
        ...
        if (
            ...
            and self.auto_episode_group in (1, 2)
            and not self.rescrape_with_episode_group
        ):
            self.rescrape_with_episode_group = True
            if self.auto_episode_group == 1 or kodi_utils.confirm_dialog(
                heading=self.meta.get("rootname", ""),
                text="No results.[CR]Retry With Custom Episode Group if Possible?",
            ):
                    ...
                    self.params.update(
                        {
                            "custom_season": None,
                            "custom_episode": None,
                            "episode_group_label": "[B]CUSTOM GROUP: S%02dE%02d[/B]"
                            % (self.season, self.episode),
                ...
                if self.auto_episode_group == 2:
                    ...
                    try:
                        group_id = episode_groups_choice(
                            {"meta": self.meta, "poster": self.meta["poster"]}
                        )
                    except:
                        group_id = None
                else:
        ...
        if self.orig_results and not self.background:
            ...
            if self.ignore_results_filter == 1 or kodi_utils.confirm_dialog(
                heading=self.meta.get("rootname", ""),
                text="No results. Access Filtered Results?",
            ):
    ...
    def get_search_title(self):
        search_title = (
            self.meta.get("custom_title", None)
            or self.meta.get("english_title")
    ...
    def get_ep_name(self):
        ...
        if self.meta["media_type"] == "episode":
            ...
            try:
                ep_name = safe_string(remove_accents(ep_name))
            except:
                ep_name = safe_string(ep_name)
        return ep_name
    ...
    def _quality_filter(self):
        setting = (
            "results_quality_%s" % self.media_type
            if not self.autoplay
            else "autoplay_quality_%s" % self.media_type
        )
    ...
    def get_meta(self):
        if self.media_type == "movie":
            ...
            try:
                self.meta = metadata.tvshow_meta(
                    "tmdb_id",
                    self.tmdb_id,
                    settings.tmdb_api_key(),
                    settings.mpaa_region(),
                ...
                episode_data = [
                    i for i in episodes_data if i["episode"] == self.episode
                ][0]
                ep_thumb = (
                    episode_data.get("thumb", None) or self.meta.get("fanart") or ""
                )
    ...
    def _clear_properties(self):
        ...
        for item in def_internal:
            kodi_utils.clear_property("bacterio.internal_results.%s" % item)
        if self.active_folders:
            for item in self.folder_info:
                kodi_utils.clear_property("bacterio.internal_results.%s" % item[0])

    def _make_progress_dialog(self):
        self.progress_dialog = create_window(
            ("windows.sources", "SourcesPlayback"),
            "sources_playback.xml",
            meta=self.meta,
        )
    ...
    def _make_nextep_dialog(self, default_action="cancel"):
        try:
            action = open_window(
                ("windows.playback_notifications", "NextEpisode"),
                "playback_notifications.xml",
                meta=self.meta,
    ...
    def _make_still_watching_dialog(self):
        try:
            action = open_window(
                ("windows.playback_notifications", "StillWatching"),
                "playback_notifications.xml",
                meta=self.meta,
            )
        except:
    ...
    def debridPacks(self, debrid_provider, name, magnet_url, info_hash, download=False):
        ...
        self._kill_progress_dialog()
        return BacterioPlayer().run(link, "video")
    ...
    def play_file(self, results, source={}):
        ...
        try:
            ...
            url = None
            results = [
                i for i in results if "Uncached" not in i.get("cache_provider", "")
            ]
            if not source:
                source = results[0]
            items = [source]
                ...
                try:
                    ...
                    if count > 1:
                        ...
                        try:
                            ...
                            pass
                    url, self.playback_successful, self.cancel_all_playback = (
                        None,
                        None,
                        False,
                    )
                    ...
                    player = BacterioPlayer()
                    try:
                        if (
                            self.progress_dialog.iscanceled()
                        ...
                        if url:
                            ...
                            self.progress_dialog.update_resolver(
    ...
    def get_playback_percent(self):
        if self.media_type == "movie":
            percent = watched_status.get_progress_status_movie(
                watched_status.get_bookmarks_movie(), str(self.tmdb_id)
            )
        elif any((self.random, self.random_continual)):
            return 0.0
        else:
            percent = watched_status.get_progress_status_episode(
                watched_status.get_bookmarks_episode(self.tmdb_id, self.season),
                self.episode,
            )
        if not percent:
            return 0.0
    ...
    def playback_failed_action(self):
        ...
        if self.prescrape and self.autoplay:
            self.resolve_dialog_made, self.prescrape, self.prescrape_sources = (
                False,
                False,
                [],
            )
    ...
    def still_watching_check(self):
        ...
        if watch_count == watching_check:
            still_watching, watch_count = self._make_still_watching_dialog(), 0
        else:
            still_watching = True
        watch_count += 1
    ...
    def autoscrape_nextep_handler(self):
        ...
        if player.isPlayingVideo():
            ...
            if not results:
                ...
                kodi_utils.notification(
                    "[B]Next Episode Ready:[/B] %s S%02dE%02d"
                    % (
                        self.meta.get("title"),
                        self.meta.get("season"),
                        self.meta.get("episode"),
            ...
            if "cache_provider" in item:
                ...
                if self.meta["media_type"] == "episode":
                    if hasattr(self, "search_info"):
                        title, season, episode, pack = (
                            self.search_info["title"],
                            self.search_info["season"],
                            self.search_info["episode"],
                            "package" in item,
                        )
                    else:
                        title, season, episode, pack = (
                ...
                url = self.resolve_internal(
                    item["scrape_provider"],
                    item["id"],
                    item["url_dl"],
                    item.get("direct_debrid_link", False),
                )
            else:
                url = item["url"]
        except:
    ...
    def resolve_cached(
        ...
        try:
            url = debrid_function().resolve_magnet(
    ...
    def resolve_internal(
        self, scrape_provider, item_id, url_dl, direct_debrid_link=False
    ):
        url = None
        try:
