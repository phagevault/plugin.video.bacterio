...
def get_external_cache_status(debrid, unchecked_hashes, data, active_debrid):
	...
	try:
		results = []
		imdb_id = data['imdb']
		debrid_name, services, token = {'Real-Debrid': ('realdebrid', ['torrentio'], get_setting('bacterio.rd.token')),
										'AllDebrid': ('alldebrid', ['mediafusion'], get_setting('bacterio.ad.token'))}[debrid]
