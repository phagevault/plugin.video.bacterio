...
def source_filters():
    return (
        ("PACK", "PACK"),
        ("DOLBY VISION", "[B]D/VISION[/B]"),
        ("HIGH DYNAMIC RANGE (HDR)", "[B]HDR[/B]"),
        ("IMAX", "IMAX"),
        ("HYBRID", "[B]HYBRID[/B]"),
        ("AV1", "[B]AV1[/B]"),
        ("HEVC (X265)", "[B]HEVC[/B]"),
        ("REMUX", "REMUX"),
        ("BLURAY", "BLURAY"),
        ("AI ENHANCED/UPSCALED", "[B]AI ENHANCED/UPSCALED[/B]"),
        ("SDR", "SDR"),
        ("3D", "[B]3D[/B]"),
        ("DOLBY ATMOS", "ATMOS"),
        ("DOLBY TRUEHD", "TRUEHD"),
...
def include_exclude_filters():
    return {
        ...
        "dv": "D/VISION",
        "av1": "AV1",
        "enhanced_upscaled": "AI ENHANCED/UPSCALED",
        "hybrid": "HYBRID",
    }
...
def make_alias_dict(meta, title):
    ...
    if country_codes:
        aliases.extend(
            [{"title": "%s %s" % (title, i), "country": ""} for i in country_codes]
        )
    return aliases
...
def supported_video_extensions():
    ...
    return [
        i for i in supported_video_extensions if i not in ("", ".zip", ".rar", ".iso")
    ]
...
def seas_ep_filter(season, episode, release_title, split=False, return_match=False):
    ...
    string_list_append(
        string8.replace("<<S>>", season_fill).replace("<<E>>", episode_fill)
    )
    string_list_append(
        string8.replace("<<S>>", str_season).replace("<<E>>", episode_fill)
    )
    string_list_append(
        string8.replace("<<S>>", season_fill).replace("<<E>>", str_episode)
    )
    string_list_append(
        string8.replace("<<S>>", str_season).replace("<<E>>", str_episode)
    )
...
def check_title(title, release_title, aliases, year, season, episode):
    try:
        ...
        if hdlr:
            ...
            if not any(release_title == i for i in cleaned_titles):
                return False
        else:
            release_title = (
                release_title.replace(year, "")
...
def get_info(title):
    ...
    if ".sdr" in title:
        ...
        for i in (
            ...
            "2160p.blu.ray.hevc.truehd",
            "2160p.blu.ray.hevc.dts",
            "2160p.uhd.bluray",
            "2160p.uhd.blu.ray",
    ...
    if any(i in title for i in (".imax.", ".(imax).", ".(.imax.).")):
        ...
        i in title for i in (".enhanced.", ".upscaled.", ".enhance.", ".upscale.")
    ):
    ...
    if any(i in title for i in ("bluray", "blu.ray", "bdrip", "bd.rip")):
        ...
        i in title for i in (".web.", "webdl", "web.dl", "web-dl", "webrip", "web.rip")
    ):
        info_append("WEB")
    elif "hdtv" in title:
        info_append("HDTV")
    ...
    if any(i in title for i in ("ch8.", "8ch.", "7.1ch", "7.1.")):
        ...
        i in title for i in ("ch2", "2ch", "2.0ch", "2.0.", "audio.2.0.", "stereo")
    ):
        info_append("2CH")
    if ".wmv" in title:
        info_append("WMV")
...
def get_cache_expiry(media_type, meta, season):
    try:
        ...
        if media_type == "movie":
            premiered = jsondate_to_datetime(
                meta["premiered"], "%Y-%m-%d", remove_time=True
            )
            ...
            if ended and last_ep_difference <= 14:
                recently_ended = True
    ...
    def _process(service, hashes):
        ...
        if service in ("mediafusion", "torrentio"):
            if service == "mediafusion":
                base_link, name_test = (
                    "https://mediafusion.elfhosted.com/%s=%s" % (debrid_name, token),
                    "⚡",
                )
                params = json.dumps(
                    {
                ...
                headers = {
                    "encoded_user_data": base64.b64encode(params).decode("utf-8")
                }
            elif service == "torrentio":
                base_link, name_test = (
                    "https://torrentio.strem.fun/%s=%s" % (debrid_name, token),
                    "[RD+]",
                )
            ...
            try:
                if "tvshowtitle" in data:
                    url = "%s%s" % (
                        ...
                        % (imdb_id, data["season"], data["episode"]),
                    )
                else:
                    url = "%s%s" % (base_link, "/stream/movie/%s.json" % imdb_id)
                result = requests.get(url, headers=headers, timeout=9)
                result = result.json()["streams"]
                if result:
                    result = [
                        re.search(r"\b\w{40}\b", i.get("url"))
                        for i in result
            ...
            def get_secret():
                ...
                s = f"{s:x}"
                n = generate_hash(
                    "debridmediamanager.com%%fe7#td00rA3vHz%VmI-" + hex_str
                )
                n = f"{n:x}"
            ...
            def fetch(hash_chunk):
                try:
                    ...
                    r = requests.post(
                        "https://debridmediamanager.com/api/availability/check",
                        json=json_data,
                        timeout=9,
                    ).json()
            ...
            threads = [
                Thread(target=fetch, args=(item,)) for item in unchecked_hashes_chunks
            ]
    ...
    try:
        ...
        debrid_name, services, token = {
            "Real-Debrid": (
                "realdebrid",
                ["torrentio"],
                get_setting("bacterio.rd.token"),
            ),
            "AllDebrid": (
                "alldebrid",
                ["mediafusion"],
                get_setting("bacterio.ad.token"),
            ),
        }[debrid]
        threads = [
            Thread(target=_process, args=(item, unchecked_hashes)) for item in services
        ]
        [i.start() for i in threads]
        [i.join() for i in threads]
