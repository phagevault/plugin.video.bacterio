...
def navigate_to_page_choice(params):
    def _builder():
        for item in start_list:
            if item == current_page:
                line1 = "[COLOR blue][B]Page %s   |   Current Page[/B][/COLOR]" % item
            else:
                line1 = "Page %s" % item
            yield {"line1": line1}
    ...
    try:
        current_page, total_pages = (
            int(params.get("current_page")),
            int(params.get("total_pages")),
        )
        ...
        kwargs = {
            ...
            "narrow_window": "true",
            "set_focus": current_page - 1,
        }
        new_page = kodi_utils.select_dialog(start_list, **kwargs)
        if new_page is None or new_page == current_page:
            return
...
def language_invoker_choice(params):
    ...
    addon_xml = kodi_utils.translate_path(
    ...
    if not kodi_utils.confirm_dialog(
        text="Turn [B]Reuse Langauage Invoker[/B] %s?"
        % ("On" if new_value == "true" else "Off")
    ):
        return
...
def context_menu_order_choice(params):
    ...
    default_settings = default_setting_values("context_menu.order")[
        "setting_default"
    ].split(",")
    removed_settings = [i for i in default_settings if i not in current_settings]
    not_default = default_settings != current_settings
    if not_default:
        start_choices = [
            {"name": "EDIT current items", "action": "edit"},
            {"name": "RESTORE to default", "action": "restore"},
        ]
        if removed_settings:
            start_choices.insert(
                0, {"name": "RE-ADD a removed item", "action": "readd"}
            )
        list_items = [{"line1": i["name"]} for i in start_choices]
        kwargs = {
            "items": json.dumps(list_items),
            "narrow_window": "true",
            "heading": "Context Menu Editor Action...",
        }
        ...
        if start_choice == "restore":
            confirm = kodi_utils.confirm_dialog(
                heading="Context Menu",
                text="Would you like to RESTORE your Context Menu to default?",
            )
            if not confirm:
                return context_menu_order_choice(params)
            set_setting(
                "context_menu.order",
                default_setting_values("context_menu.order")["setting_default"],
            )
        ...
        start_choice = "edit"
    current_choices = [
        (options[i], i, current_settings.index(i)) for i in current_settings
    ]
    ...
    if start_choice == "edit":
        ...
        action_choices = [
            {"name": "Remove [B]%s[/B]" % current_display_name, "action": "remove"},
            {"name": "Move [B]%s[/B]" % current_display_name, "action": "move"},
        ]
        list_items = [{"line1": i["name"]} for i in action_choices]
        kwargs = {
            "items": json.dumps(list_items),
            "narrow_window": "true",
            "heading": "Chose Action...",
        }
        ...
        if action_choice == "remove":
            ...
            kodi_utils.ok_dialog(
                text="[B]%s[/B] removed from Context Menu" % current_display_name
            )
            return context_menu_order_choice(params)
    ...
    kwargs = kwargs = {
        "items": json.dumps(list_items),
        "heading": "Move %s to New Position" % item_choice[0],
        "narrow_window": "true",
    }
    move_choice = kodi_utils.select_dialog(
        [list_items.index(i) for i in list_items], **kwargs
    )
    if move_choice is None:
        return context_menu_order_choice(params)
    if start_choice == "edit":
        current_settings.remove(current_name)
...
def personallists_manager_choice(params):
    from indexers.personal_lists import (
        get_all_personal_lists,
        make_new_personal_list,
        new_list_check,
    )
    ...
    all_lists = get_all_personal_lists(
        get_setting("bacterio.personal_list.list_sort", "0")
    )
    choices = []
    if not all_lists:
        action = "add_new"
        ...
        choices = [
            ("Add To Personal List...", "add"),
            ("Remove From Personal List...", "remove"),
            ("Add To [B]NEW[/B] Personal List...", "add_new"),
        ]
    ...
    if action == "add_new":
        ...
        if not list_name:
            return kodi_utils.notification("Error Creating List", 3000)
        action = "add"
    else:
        new_template, normal_template = (
            "[COLOR FF008EB2]%s [I](x%02d)[/I][/COLOR]",
            "%s [I](x%02d)[/I]",
        )
        choices = [
            (
                (new_template if new_list_check(i["seen"]) else normal_template)
                % (i["name"], i["total"]),
                (i["name"], i["author"]),
            )
            for i in all_lists
    ...
    if action == "add":
        new_contents = {
            ...
            "title": params["title"],
            "type": list_type,
            "release_date": params["premiered"],
            "date_added": params["current_time"],
        }
    else:
        new_contents = params["tmdb_id"]
    from caches.personal_lists_cache import personal_lists_cache

    result = personal_lists_cache.add_remove_list_item(
        list_name, author, action, new_contents
    )
    kodi_utils.notification(result, 3000)
    if action == "remove" and any(
        [kodi_utils.path_check(list_name) or kodi_utils.external()]
    ):
...
def tmdblists_manager_choice(params):
    ...
    if not all_lists:
        ...
        choices = [
            ("Add To TMDb List...", "add"),
            ("Remove From TMDb List...", "remove"),
            ("Add To [B]NEW[/B] TMDb List...", "add_new"),
        ]
        list_items = [{"line1": item[0], "icon": icon} for item in choices]
    ...
    if action == "add_new":
        ...
        if not list_id:
            return kodi_utils.notification("Error Creating List")
        action = "add"
    else:
        choices = [
            ("%s [I](x%02d)[/I]" % (i["name"], i["number_of_items"]), i["id"])
            for i in all_lists
        ]
        list_items = [{"line1": i[0]} for i in choices]
        kwargs = {"items": json.dumps(list_items), "narrow_window": "true"}
        ...
        if list_id is None:
            return
    new_contents = {
        "items": [{"media_type": params["media_type"], "media_id": params["tmdb_id"]}]
    }
    item_in_list = check_item_status(list_id, params["media_type"], params["tmdb_id"])
    if action == "add":
        if item_in_list:
            return kodi_utils.notification("Item already in List")
        add_to_tmdb_list(list_id, new_contents)
    else:
        if not item_in_list:
            return kodi_utils.notification("Item not in List")
    ...
    if action == "remove" and any(
        [kodi_utils.path_check(str(list_id)) or kodi_utils.external()]
    ):
        kodi_utils.kodi_refresh()
...
def favorites_manager_choice(params):
    ...
    if current_favorite:
        ...
        if param_refresh is None:
            refresh = any(
                i in kodi_utils.folder_path()
                for i in (
                    "action=favorites_movies",
                    "action=favorites_tvshows",
                    "action=favorites_anime",
                )
            )
        else:
            refresh = param_refresh == "true"
    else:
        function, text, refresh = (
            favorites_cache.set_favourite,
            "Add To Favorites?",
            False,
        )
    heading = title.split("|")[0] if people_favorite else title
    if not kodi_utils.confirm_dialog(heading=heading, text=text):
        return
    ...
    if people_favorite and success:
        return text


def ai_model_order_choice(params):
    model_descriptions = {
        "gemini-2.5-flash-lite": ("GEMINI FAST, 20 RPD", "gemini"),
        "llama-3.3-70b-versatile": ("GROQ FAST, 140 RPD", "groq"),
        "gemma-3-27b-it": ("GEMMA Fast, MANY RPD", "gemma"),
    ...
    choices = [
        {
            "line1": "Position %02d" % (count + 1),
            "line2": "Currently [B]%s[/B] (%s)" % (item, model_descriptions[item][0]),
            "icon": kodi_utils.get_icon(model_descriptions[item][1]),
            "current_item": item,
            "display_position": count + 1,
            "position": count,
        }
        for count, item in enumerate(current_order)
    ]
    kwargs = {
        "items": json.dumps(choices),
        "multi_line": "true",
        "heading": "Choose Sort Order Of AI Models",
    }
    choice = kodi_utils.select_dialog(choices, **kwargs)
    if choice is None:
        return
    ...
    choices = [
        {
            "line1": item,
            "line2": model_descriptions[item][0],
            "icon": kodi_utils.get_icon(model_descriptions[item][1]),
            "model_id": item,
        }
        for item in default_order
        if item != current_model_id
    ]
    kwargs = {
        "items": json.dumps(choices),
        "multi_line": "true",
        "heading": "Choose Model for Position %02d" % display_position,
    }
    choice = kodi_utils.select_dialog(choices, **kwargs)
...
def preferred_filters_choice(params):
    ...
    def _default_choices():
        return [
            {"name": "1st Sort", "value": "Choose 1st Sort Param"},
            {"name": "2nd Sort", "value": "Choose 2nd Sort Param"},
            {"name": "3rd Sort", "value": "Choose 3rd Sort Param"},
            {"name": "4th Sort", "value": "Choose 4th Sort Param"},
            {"name": "5th Sort", "value": "Choose 5th Sort Param"},
        ]

    def _beginning_choices():
        defaults = _default_choices()
        for count, item in enumerate(auto_settings):
            defaults[count]["value"] = item
        return defaults

    def _rechoose_checker(choice):
        if choice["value"].startswith("Choose"):
            return (choice, True)
        clear_choice = kodi_utils.confirm_dialog(
            heading="Current Param Active",
            text="This sort slot is already filled.[CR]Please choose what action to take.",
            ok_label="Remake Slot",
            cancel_label="Clear Slot",
        )
        if clear_choice is None:
            new_default, ask_params = (choice, False)
        else:
    ...
    def _param_choices(choice):
        ...
        disabled_filters = [
            v for k, v in filter_keys.items() if settings.filter_status(k) == 1
        ]
        s_filters = source_filters()
        filters_choice = [
            (i[0], i[1].replace("[B]", "").replace("[/B]", "")) for i in s_filters
        ]
        filters_choice = [i for i in filters_choice if i[1] not in disabled_filters]
        unused_filters = [i for i in filters_choice if i[1] not in auto_settings]
        param_list_items = [{"line1": i[0], "line2": i[1]} for i in unused_filters]
        param_kwargs = {
            "items": json.dumps(param_list_items),
            "multi_line": "true",
            "heading": "Choose Sort to Top Params for Autoplay",
            "narrow_window": "true",
        }
    ...
    def _make_settings():
        new_settings = [
            i["value"] for i in choices if not i["value"].startswith("Choose")
        ]
        if not new_settings:
            set_setting("filter.preferred_filters", "empty_setting")
        else:
            set_setting("filter.preferred_filters", ", ".join(new_settings))
...
def tmdb_api_check_choice(params):
    ...
    if not data.get("success", True):
        text = 'There is an issue with your API Key.[CR][B]"Error: %s"[/B]' % data.get(
            "status_message", ""
        )
    else:
        text = "Your TMDb API Key is enabled and working"
...
def clear_sources_folder_choice(params):
    ...
    set_default(
        [
            "%s.display_name" % setting_id,
            "%s.movies_directory" % setting_id,
            "%s.tv_shows_directory" % setting_id,
        ]
    )
...
def widget_refresh_timer_choice(params):
    ...
    choices.extend(
        [{"name": "Every %s Minutes" % i, "value": str(i)} for i in range(5, 25, 5)]
    )
    choices.extend(
        [{"name": "Every %s Minutes" % i, "value": str(i)} for i in range(30, 65, 10)]
    )
    choices.extend(
        [
            {"name": "Every %s Hours" % (float(i) / 60), "value": str(i)}
            for i in range(90, 720, 30)
        ]
    )
...
def limit_number_quality_choice(params):
    choices = [{"name": "OFF", "value": "0"}]
    choices.extend(
        [{"name": "%sx Per Quality" % i, "value": str(i)} for i in range(1, 5)]
    )
    choices.extend(
        [{"name": "%sx Per Quality" % i, "value": str(i)} for i in range(5, 205, 5)]
    )
    list_items = [{"line1": i["name"]} for i in choices]
    kwargs = {"items": json.dumps(list_items), "narrow_window": "true"}
    choice = kodi_utils.select_dialog(choices, **kwargs)
...
def limit_number_total_choice(params):
    choices = [{"name": "OFF", "value": "0"}]
    choices.extend(
        [{"name": "%sx Total Results" % i, "value": str(i)} for i in range(1, 10)]
    )
    choices.extend(
        [{"name": "%sx Total Results" % i, "value": str(i)} for i in range(10, 1000, 5)]
    )
    list_items = [{"line1": i["name"]} for i in choices]
    kwargs = {"items": json.dumps(list_items), "narrow_window": "true"}
    choice = kodi_utils.select_dialog(choices, **kwargs)
...
def external_scraper_choice(params):
    ...
    try:
        ...
        sourceDict = manual_function_import(main_folder_name, "sources")(
    ...
    if success:
        ...
        kodi_utils.ok_dialog(
            text="The [B]%s[/B] Module is not compatible.[CR]Please choose a different Module..."
            % module_name.upper()
        )
...
def audio_filters_choice(params={}):
    ...
    list_items = [
        {"line1": item[0], "line2": item[1], "icon": icon} for item in audio_filters
    ]
    try:
        preselect = [
            audio_filters.index(item)
    ...
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Choose Audio Properties to Exclude",
        "multi_choice": "true",
        "multi_line": "true",
        "preselect": preselect,
    }
    ...
    if selection is None:
        return
    if selection == []:
        set_setting("filter_audio", "empty_setting")
    else:
        set_setting("filter_audio", ", ".join(selection))


def genres_choice(params):
    genres_list, genres, poster = (
        params["genres_list"],
        params["genres"],
        params["poster"],
    )
...
def keywords_choice(params):
    ...
    keywords, tmdb_id, poster = (
        meta.get("keywords", []),
        meta["tmdb_id"],
        meta["poster"],
    )
    if keywords:
        keywords = keywords.get("keywords") or keywords.get("results")
    else:
        kodi_utils.show_busy_dialog()
        from apis.tmdb_api import tmdb_movie_keywords, tmdb_tv_keywords

        if media_type == "movie":
            function, key = tmdb_movie_keywords, "keywords"
        else:
...
def random_choice(params):
    meta, poster, return_choice = (
        params.get("meta"),
        params.get("poster"),
        params.get("return_choice", "false"),
    )
    meta = params.get("meta", None)
    list_items = [
        {"line1": "Single Random Play", "icon": poster},
        {"line1": "Continual Random Play", "icon": poster},
    ]
    choices = ["play_random", "play_random_continual"]
    kwargs = {"items": json.dumps(list_items), "heading": "Choose Random Play Type..."}
    choice = kodi_utils.select_dialog(choices, **kwargs)
    if return_choice == "true":
        return choice
    if choice is None:
        return
...
def trakt_manager_choice(params):
    ...
    tmdb_id, tvdb_id, imdb_id, media_type = (
        params["tmdb_id"],
        params["tvdb_id"],
        params["imdb_id"],
        params["media_type"],
    ...
    if media_type == "movie":
        ...
        media_id, media_key = next(
            item for item in media_ids if item[0] not in ("None", None, "")
        )
        if media_id in (tmdb_id, tvdb_id):
            media_id = int(media_id)
    data = {key: [{"ids": {media_key: media_id}}]}
    if choice == "add_watchlist":
        return trakt_api.add_to_watchlist(data)
    if choice == "remove_watchlist":
        return trakt_api.remove_from_watchlist(data)
    if choice == "add_collection":
        return trakt_api.add_to_collection(data)
    if choice == "remove_collection":
        return trakt_api.remove_from_collection(data)
    selected = trakt_api.get_trakt_list_selection(["personal"])
    if selected is None:
        return
    trakt_api.add_to_list(
        selected["user"], selected["slug"], data
    ) if choice == "add" else trakt_api.remove_from_list(
        selected["user"], selected["slug"], data
    )
...
def episode_groups_choice(params):
    ...
    list_items = [
        {
            "line1": "%s | %s Order | %d Groups | %02d Episodes"
            % (
                item["name"],
                episode_group_types[item["type"]],
                item["group_count"],
                item["episode_count"],
            ),
...
def assign_episode_group_choice(params):
    ...
    if current_group:
        action = kodi_utils.confirm_dialog(
            text="Set new Group or Clear Current Group?",
            ok_label="Set New",
            cancel_label="Clear",
            default_control=10,
        )
...
def playback_choice(params):
    ...
    media_type, season, episode, episode_id = (
        params.get("media_type"),
        params.get("season", ""),
        params.get("episode", ""),
        params.get("episode_id", None),
    )
    ...
    if not isinstance(meta, dict):
        function = (
            metadata.movie_meta if media_type == "movie" else metadata.tvshow_meta
        )
        meta = function(
            "tmdb_id",
            meta,
            settings.tmdb_api_key(),
            settings.mpaa_region(),
            get_datetime(),
        )
    poster = meta.get("poster") or kodi_utils.get_icon("box_office")
    aliases = get_aliases_titles(make_alias_dict(meta, meta["title"]))
    check_cache_status, check_cache_toggle = (
        ("OFF", "false") if settings.external_cache_check() else ("ON", "true")
    )
    items = [
        {"line": "Select Source", "function": "scrape"},
        {"line": "Rescrape & Select Source", "function": "clear_and_rescrape"},
    ]
    if debrid_for_ext_cache_check():
        items.append(
            {
                "line": "Rescrape with External Cache Check [B]%s[/B]"
                % check_cache_status,
                "function": "rescrape_external_cache_check",
            }
        )
    items.extend(
        [
            {
                "line": "Clear Debrid Cache & Show Results",
                "function": "clear_debrid_cache_and_show",
            },
            {
                "line": "Scrape with ALL External Scrapers",
                "function": "scrape_with_disabled",
            },
            {
                "line": "Scrape With All Filters Ignored",
                "function": "scrape_with_filters_ignored",
            },
        ]
    )
    if media_type == "episode":
        items.append(
            {
                "line": "Scrape with Custom Episode Groups Value",
                "function": "scrape_with_episode_group",
            }
        )
    if aliases:
        items.append(
            {"line": "Scrape with an Alias", "function": "scrape_with_aliases"}
        )
    items.append(
        {"line": "Scrape with Custom Values", "function": "scrape_with_custom_values"}
    )
    ...
    if choice is None:
        return kodi_utils.notification("Cancelled", 2500)
    ...
    if choice == "scrape":
        ...
        episode_details = metadata.group_episode_data(
            metadata.group_details(choice), episode_id, season, episode
        )
        ...
        play_params = {
            ...
            "episode": episode,
            "prescrape": "false",
            "custom_season": episode_details["season"],
            "custom_episode": episode_details["episode"],
        }
    elif choice == "scrape_with_aliases":
        if len(aliases) == 1:
            custom_title = aliases[0]
        else:
        ...
        if aliases:
            ...
            if alias_title:
                custom_title = kodi_utils.kodi_dialog().input(
                    "Title", defaultt=alias_title
                )
            else:
                custom_title = kodi_utils.kodi_dialog().input(
                    "Title", defaultt=default_title
                )
        else:
            custom_title = kodi_utils.kodi_dialog().input(
                "Title", defaultt=default_title
        ...
        def _process_params(default_value, custom_value, param_value):
            if custom_value and custom_value != default_value:
                play_params[param_value] = custom_value

        _process_params(default_title, custom_title, "custom_title")
        custom_year = kodi_utils.kodi_dialog().input(
            "Year", type=1, defaultt=default_year
        )
        _process_params(default_year, custom_year, "custom_year")
        if media_type == "episode":
            custom_season = kodi_utils.kodi_dialog().input(
                "Season", type=1, defaultt=season
            )
            _process_params(season, custom_season, "custom_season")
            custom_episode = kodi_utils.kodi_dialog().input(
                "Episode", type=1, defaultt=episode
            )
            _process_params(episode, custom_episode, "custom_episode")
            if any(i in play_params for i in ("custom_season", "custom_episode")):
                if settings.autoplay_next_episode():
                    _process_params("", "true", "disable_autoplay_next_episode")
        all_choice = kodi_utils.confirm_dialog(
            heading=meta.get("rootname", ""),
            text="Scrape with ALL External Scrapers?",
            ok_label="Yes",
            cancel_label="No",
        )
        if all_choice is None:
            return kodi_utils.notification("Cancelled", 2500)
        if all_choice:
            _process_params("", "true", "disabled_ext_ignored")
        disable_filters_choice = kodi_utils.confirm_dialog(
            heading=meta.get("rootname", ""),
            text="Disable All Filters for Search?",
            ok_label="Yes",
            cancel_label="No",
        )
        if disable_filters_choice is None:
            return kodi_utils.notification("Cancelled", 2500)
        if disable_filters_choice:
            _process_params("", "true", "ignore_scrape_filters")
            kodi_utils.set_property("fs_filterless_search", "true")
    if media_type == "episode":
        play_params["playcount"] = playcount
...
def extras_buttons_choice(params):
    ...
    media_type, button_dict, orig_button_dict = (
        params.get("media_type", None),
        params.get("button_dict", {}),
        params.get("orig_button_dict", {}),
    )
        ...
        for _type in ("movie", "tvshow"):
            ...
            for item in range(10, 18):
                ...
                button_dict[setting_id] = {
                    ...
                    "button_name": "Button %s" % str(item - 9),
                }
    ...
    if media_type is None:
        ...
        kwargs = {
            "items": json.dumps(list_items),
            "heading": "Choose Media Type to Set Buttons",
            "narrow_window": "true",
        }
        choice = kodi_utils.select_dialog(choices, **kwargs)
        if choice is None:
        ...
        if "restore" in media_type:
            ...
            if restore_type in ("movie", "both"):
                for item in [
                    (i, default_setting_values(i)["setting_default"])
                    for i in ("extras.movie.button%s" % i for i in range(10, 18))
                ]:
                    set_setting(item[0], item[1])
            if restore_type in ("tvshow", "both"):
                for item in [
                    (i, default_setting_values(i)["setting_default"])
                    for i in ("extras.tvshow.button%s" % i for i in range(10, 18))
                ]:
                    set_setting(item[0], item[1])
            return extras_buttons_choice({})
    choices = [
        (
            "[B]%s[/B]   |   %s" % (v["button_name"], v["button_label"]),
            v["button_name"],
            v["button_label"],
            k,
        )
    ...
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Choose Button to Set",
        "narrow_window": "true",
    }
    choice = kodi_utils.select_dialog(choices, **kwargs)
    if choice is None:
        return extras_buttons_choice(
            {"button_dict": button_dict, "orig_button_dict": orig_button_dict}
        )
    button_name, button_label, button_setting = choice[1:]
    choices = [
        (v, k)
        for k, v in extras_button_label_values[media_type].items()
        if not v == button_label
    ]
    choices = [i for i in choices if not i[0] == button_label]
    list_items = [{"line1": i[0]} for i in choices]
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Choose Action For %s" % button_name,
        "narrow_window": "true",
    }
    choice = kodi_utils.select_dialog(choices, **kwargs)
    if choice is None:
        return extras_buttons_choice(
            {
                ...
                "orig_button_dict": orig_button_dict,
                "media_type": media_type,
            }
        )
    button_label, button_action = choice
    button_dict[button_setting] = {
        "button_action": button_action,
        "button_label": button_label,
...
def extras_lists_choice(params={}):
    ...
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Enable Content for Extras Lists",
        "multi_choice": "true",
        "preselect": preselect,
    }
    selection = kodi_utils.select_dialog(choices, **kwargs)
    if selection == []:
        return set_setting("extras.enabled", "noop")
...
def set_language_filter_choice(params):
    ...
    filter_setting_id, multi_choice, include_none = (
        params.get("filter_setting_id"),
        params.get("multi_choice", "false"),
        params.get("include_none", "false"),
    )
    ...
    if multi_choice == "true":
        if choice == []:
            set_setting(filter_setting_id, "eng")
        else:
            set_setting(filter_setting_id, ", ".join(choice))
    else:
...
def sources_folders_choice(params):
    ...
    return open_window(
        ("windows.settings_manager", "SettingsManagerFolders"),
        "settings_manager_folders.xml",
    )
...
def results_sorting_choice(params={}):
    choices = [
        ...
        ("Provider, Quality, Size", "2"),
        ("Provider, Size, Quality", "3"),
        ("Size, Quality, Provider", "4"),
        ("Size, Provider, Quality", "5"),
    ]
...
def scraper_color_choice(params):
    ...
    current_setting, original_highlight = (
        get_setting("bacterio.%s" % setting),
        default_setting_values(setting)["setting_default"],
    )
    if current_setting != original_highlight:
        action = kodi_utils.confirm_dialog(
            text="Set new Highlight or Restore Default Highlight?",
            ok_label="Set New",
            cancel_label="Restore Default",
            default_control=10,
        )
        if action is None:
            return
        if not action:
            return set_setting(setting, original_highlight)
...
def color_choice(params):
    ...
    return open_window(
        ("windows.color", "SelectColor"),
        "color.xml",
        current_setting=params.get("current_setting", None),
    )
...
def lists_cache_duration_choice(params={}):
    durations = [
        ...
        {"name": "18 hours", "duration": "18"},
        {"name": "1 Day", "duration": "24"},
        {"name": "2 Days", "duration": "48"},
        {"name": "3 Days", "duration": "72"},
        {"name": "4 Days", "duration": "96"},
        {"name": "5 Days", "duration": "120"},
        {"name": "6 Days", "duration": "144"},
        {"name": "7 Days", "duration": "168"},
    ]
    list_items = [{"line1": i["name"]} for i in durations]
    kwargs = {
        "items": json.dumps(list_items),
        "heading": "Set Generic List Cache Duration",
        "narrow_window": "true",
    }
...
def options_menu_choice(params, meta=None):
    ...
    tmdb_id, content, poster = (
        params_get("tmdb_id", None),
        params_get("content", None),
        params_get("poster", None),
    )
    is_external, from_extras = (
        params_get("is_external") in (True, "True", "true"),
        params_get("from_extras", "false") == "true",
    )
    ...
    single_ep_list = (
        ...
        "episode.next_trakt",
        "episode.next_bacterio",
        "episode.trakt_recently_aired",
        "episode.trakt_calendar",
    )
    if not content:
        content = kodi_utils.container_content()[:-1]
    menu_type = content
    if content.startswith("episode."):
        content = "episode"
    if not meta:
    ...
    meta_get = meta.get
    rootname, title, imdb_id, tvdb_id = (
        meta_get("rootname", None),
        meta_get("title"),
        meta_get("imdb_id", None),
        meta_get("tvdb_id", None),
    )
    window_function = (
        kodi_utils.activate_window if is_external else kodi_utils.container_update
    )
    listing = []
    ...
    if from_extras:
        ...
        if settings.trakt_user_active():
            listing_append(("Trakt Lists Manager", "", "trakt_manager"))
    ...
    if menu_type == "tvshow":
        listing_append(("Play Random", "Based On %s" % rootname, "random"))
    if menu_type in ("tvshow", "season"):
        listing_append(
            (
                "Assign an Episode Group to %s" % rootname,
                "Currently %s" % episode_groups_cache.get(tmdb_id).get("name", "None"),
                "episode_group",
            )
        )
    if menu_type in ("movie", "episode") or menu_type in single_ep_list:
        base_str1, base_str2, on_str, off_str = (
            "%s%s",
            "Currently: [B]%s[/B]",
            "On",
            "Off",
        ...
        if settings.auto_play(content):
            autoplay_status, autoplay_toggle, quality_setting = (
                on_str,
                "false",
                "autoplay_quality_%s" % content,
            )
        else:
            autoplay_status, autoplay_toggle, quality_setting = (
                off_str,
                "true",
                "results_quality_%s" % content,
            )
        set_active = settings.active_internal_scrapers()
        active_int_scrapers = [i.replace("_", "") for i in set_active]
        current_scrapers_status = (
            ", ".join([i for i in active_int_scrapers])
            if len(active_int_scrapers) > 0
            else "N/A"
        )
        current_quality_status = ", ".join(settings.quality_filter(quality_setting))
        autoplay_next_status, autoplay_next_toggle = (
            (on_str, "false") if settings.autoplay_next_episode() else (off_str, "true")
        )
        listing_append(
            (
                base_str1 % ("Auto Play", " (%s)" % content),
                base_str2 % autoplay_status,
                "toggle_autoplay",
            )
        )
        if menu_type == "episode" or menu_type in single_ep_list:
            if autoplay_status == on_str:
                ...
                listing_append(
                    (
                        base_str1 % ("Autoplay Next Episode", ""),
                        base_str2 % autoplay_next_status,
                        "toggle_autoplay_next",
                    )
                )
            else:
                autoscrape_next_status, autoscrape_next_toggle = (
                    (on_str, "false")
                    if settings.autoscrape_next_episode()
                    else (off_str, "true")
                )
                listing_append(
                    (
                        base_str1 % ("Autoscrape Next Episode", ""),
                        base_str2 % autoscrape_next_status,
                        "toggle_autoscrape_next",
                    )
                )
        listing_append(
            (
                base_str1 % ("Quality Limit", " (%s)" % content),
                base_str2 % current_quality_status,
                "set_quality",
            )
        )
        listing_append(
            (
                base_str1 % ("", "Enable Scrapers"),
                base_str2 % current_scrapers_status,
                "enable_scrapers",
            )
        )
        if menu_type == "episode" or menu_type in single_ep_list:
            listing_append(
                (
                    "Assign an Episode Group to %s" % rootname,
                    base_str2 % episode_groups_cache.get(tmdb_id).get("name", "None"),
                    "episode_group",
                )
            )
    if not from_extras:
        if menu_type in ("movie", "tvshow"):
            listing_append(
                (
                    "Re-Cache %s Info"
                    % ("Movies" if menu_type == "movie" else "TV Shows"),
                    "Clear %s Cache" % rootname,
                    "clear_media_cache",
                )
            )
        if menu_type in ("movie", "episode") or menu_type in single_ep_list:
            listing_append(("Clear Scrapers Cache", "", "clear_scrapers_cache"))
        if menu_type in ("tvshow", "season", "episode"):
            listing_append(("TV Shows Progress Manager", "", "nextep_manager"))
        listing_append(("Open Download Manager", "", "open_download_manager"))
        listing_append(("Open Tools", "", "open_tools"))
        if menu_type in ("movie", "episode") or menu_type in single_ep_list:
            listing_append(
                ("Open External Scraper Settings", "", "open_external_scraper_settings")
            )
        listing_append(("Open Settings", "", "open_settings"))
    list_items = [
        {"line1": item[0], "line2": item[1] or item[0], "icon": poster}
        for item in listing
    ]
    ...
    if choice == "personallists_manager_choice":
        ...
        return personallists_manager_choice(
            {
                "list_type": content,
                "tmdb_id": tmdb_id,
                "title": title,
    ...
    if choice == "favorites_manager_choice":
        return favorites_manager_choice(
            {
                "media_type": content if content in ("movie", "tvshow") else "tvshow",
                "tmdb_id": tmdb_id,
                "title": title,
            }
        )
    if choice == "tmdblists_manager_choice":
        return tmdblists_manager_choice(
            {
                "media_type": content if content in ("movie", "movies") else "tv",
                "tmdb_id": tmdb_id,
                "icon": poster,
            }
        )
    if choice == "toggle_autoplay":
        ...
        set_quality_choice(
            {
                "setting_id": "autoplay_quality_%s" % content
                if autoplay_status == on_str
                else "results_quality_%s" % content,
                "icon": poster,
            }
        )
...
def extras_menu_choice(params):
    ...
    open_window(
        ...
        "extras.xml",
        meta=meta,
        is_external=params.get(
            "is_external", "true" if kodi_utils.external() else "false"
        ),
        options_media_type=media_type,
...
def open_movieset_choice(params):
    kodi_utils.hide_busy_dialog()
    window_function = (
        kodi_utils.activate_window
        if params["is_external"] in (True, "True", "true")
        else kodi_utils.container_update
    )
    return window_function(
        {
            "mode": "build_movie_list",
            "action": "tmdb_movies_sets",
...
def media_extra_info_choice(params):
    ...
    try:
        if media_type == "movie":
            if meta["tagline"]:
                append("[B]Tagline:[/B] %s" % meta["tagline"])
            aliases = get_aliases_titles(make_alias_dict(meta, meta["title"]))
            if aliases:
                append("[B]Aliases:[/B] %s" % ", ".join(aliases))
            append("[B]Status:[/B] %s" % extra_info["status"])
            append("[B]Premiered:[/B] %s" % meta["premiered"])
            append(
                "[B]Rating:[/B] %s (%s Votes)"
                % (str(round(meta["rating"], 1)), meta["votes"])
            )
            ...
            if extra_info["collection_name"]:
                append("[B]Collection:[/B] %s" % extra_info["collection_name"])
            ...
            try:
                ...
                append(
                    "[B]Last Aired:[/B] %s - [B]S%.2dE%.2d[/B] - %s"
                    % (
                        adjust_premiered_date(
                            last_ep["air_date"], settings.date_offset()
                        )[0].strftime("%d %B %Y"),
            ...
            try:
                ...
                append(
                    "[B]Next Aired:[/B] %s - [B]S%.2dE%.2d[/B] - %s"
                    % (
                        adjust_premiered_date(
                            next_ep["air_date"], settings.date_offset()
                        )[0].strftime("%d %B %Y"),
