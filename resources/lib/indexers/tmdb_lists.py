...
def get_tmdb_lists(params):
    def get_custom_image(list_name, image_type, images):
        try:
            ...
            custom_image = [i for i in images if i.rsplit("_", 1)[0] == md5_image_name][
                0
            ]
            return os.path.join(
        ...
        for item in data:
            try:
                ...
                if custom_fanart:
                    ...
                    fanart = background
                mode = (
                    "random.build_tmdb_lists_contents"
                    if random
                    else "tmdblist.build_tmdb_list"
                )
                url_params = {
                    "mode": mode,
                    "list_id": list_id,
                ...
                cm = [
                    (
                        "[B]Make New List[/B]",
                        "RunPlugin(%s)"
                        % build_url({"mode": "tmdblist.make_new_tmdb_list"}),
                    ),
                    (
                        "[B]Edit Properties[/B]",
                        "RunPlugin(%s)"
                    ...
                    (
                        ...
                        "RunPlugin(%s)"
                        % build_url(
                            {"mode": "tmdblist.delete_tmdb_list", "list_id": list_id}
                        ),
                    ),
                    (
                        "[B]Clear Contents Cache[/B]",
                        "RunPlugin(%s)"
                        % build_url(
                            {
                                "mode": "tmdblist.cache_delete_list_tmdb",
                                "list_id": list_id,
                            }
                        ),
                    ),
                    (
                        "[B]Clear All Lists Cache[/B]",
                        "RunPlugin(%s)"
                        % build_url({"mode": "tmdblist.cache_delete_all_tmdb"}),
                    ),
                    (
    ...
    handle, icon, background = (
        int(sys.argv[1]),
        kodi_utils.get_icon("tmdb"),
        kodi_utils.get_addon_fanart(),
    )
    tmdb_image_url = "https://image.tmdb.org/t/p/%s%s"
    profile_path = kodi_utils.addon_profile()
    all_posters = kodi_utils.list_dirs(
        os.path.join(profile_path, "images", "tmdb_lists_poster")
    )[1]
    all_fanart = kodi_utils.list_dirs(
        os.path.join(profile_path, "images", "tmdb_lists_fanart")
    )[1]
    ...
    try:
        data = get_all_tmdb_lists(get_setting("bacterio.tmdblist.list_sort", "0"))
        if data:
            if shuffle_lists:
                returning_to_list = (
                    "build_tmdb_lists_contents" in kodi_utils.folder_path()
                )
                if returning_to_list:
                    ...
                    kodi_utils.set_property(
                        "bacterio.tmdb.lists.order", json.dumps(data)
                    )
            else:
                kodi_utils.clear_property("bacterio.tmdb.lists.order")
...
def build_tmdb_list(params):
    ...
    try:
        ...
        list_name, list_id, sort_order, updated_at = (
        ...
        if use_result:
            ...
            result = get_tmdb_list(params)
        result, total_pages, paginate_start = _paginate_list(
        ...
        movie_list = {
            "list": [(i["order"], i["id"]) for i in all_movies],
            "custom_order": "true",
        }
        tvshow_list = {
            "list": [(i["order"], i["id"]) for i in all_tvshows],
            "custom_order": "true",
        }
        content = max(
            [("movies", len(all_movies)), ("tvshows", len(all_tvshows))],
            key=lambda k: k[1],
        )[0]
...
def adjust_tmdb_list_properties(params):
    ...
    original_list_name, original_sort_order = (
        params.get("original_list_name", ""),
        params.get("original_sort_order", ""),
    )
    custom_poster, custom_fanart = (
        params.get("custom_poster", ""),
        params.get("custom_fanart", ""),
    )
    current_name, current_sort_order = (
        params.get("list_name", "") or original_list_name,
        sort_order or original_sort_order,
    )
    choices = [
        ("Change Name", "Currently [B]%s[/B]" % (current_name), "list_name"),
        (
            "Change Sort Order",
            "Currently [B]%s[/B]" % sort_order_dict.get(current_sort_order, "None"),
            "sort_order",
        ),
    ...
    choices.extend(
        [
            (
                "Empty List Contents",
                "Delete All Contents of %s" % current_name,
                "empty_contents",
            ),
            (
                "Import Trakt List",
                "Import a Trakt List into %s" % current_name,
                "import_trakt",
            ),
        ]
    )
    ...
    kwargs = {
        ...
        "heading": "TMDb List Properties",
        "multi_line": "true",
        "narrow_window": "true",
    }
    action = kodi_utils.select_dialog([i[2] for i in choices], **kwargs)
    if action == None:
        if params.get("refresh_cache", "false") == "true":
            cache_delete_all_tmdb()
    ...
    if action in ("make_poster", "make_fanart"):
        ...
        shuffle_sort_order = kodi_utils.confirm_dialog(
            heading="TMDb Lists",
    ...
    if action == "list_name":
        ...
        new_poster = tmdb_image_maker(
            current_name, list_id, "poster", custom_poster, shuffle_sort_order
        )
        if new_poster is None:
            return adjust_tmdb_list_properties(params)
        params.update({"custom_poster": new_poster, "refresh": "true"})
    elif action == "make_fanart":
        new_fanart = tmdb_image_maker(
            current_name, list_id, "fanart", custom_fanart, shuffle_sort_order
        )
        ...
        if not clear_tmdb_list(current_name, list_id):
            return adjust_tmdb_list_properties(params)
...
def tmdb_image_maker(list_name, list_id, image_type, custom_image, shuffle_sort_order):
    ...
    if shuffle_sort_order:
        shuffle(content)
    images = []
    if image_type == "poster":
        image_dimension, image_key = "w780", "poster_path"
    else:
        image_dimension, image_key = "w1280", "backdrop_path"
    for item in content:
        if item[image_key]:
            images.append(
                "https://image.tmdb.org/t/p/%s%s" % (image_dimension, item[image_key])
            )
        if len(images) == 4:
            break
...
def rename_tmdb_list(current_name, list_id):
    list_name = kodi_utils.kodi_dialog().input(
        "Please Choose a Name for the New List", defaultt=current_name
    )
    if list_name == None:
        return None
...
def make_new_tmdb_list(params):
    ...
    list_name = kodi_utils.kodi_dialog().input(
        "Please Choose a Name for the New TMDb List", defaultt=suggested_list_name
    )
    if not list_name:
...
def delete_tmdb_list(params):
    if not kodi_utils.confirm_dialog(
        heading="TMDb Lists", text="Are You Sure?", ok_label="Yes", cancel_label="No"
    ):
        return
    ...
    try:
        if sort_order:
            if sort_order in ("", "0", "None"):
                ...
                contents.sort(
                    key=lambda k: (k["updated_at"] is None, k["updated_at"]),
                    reverse=reverse,
                )
            elif sort_order in ("5", "6"):
                reverse = sort_order != "5"
                contents.sort(
                    key=lambda k: (k["number_of_items"] is None, k["number_of_items"]),
                    reverse=reverse,
                )
            elif sort_order in ("7", "8"):
                reverse = sort_order != "7"
                contents.sort(
                    key=lambda k: (k["average_rating"] is None, k["average_rating"]),
                    reverse=reverse,
                )
            elif sort_order in ("9", "10"):
                reverse = sort_order != "9"
                contents.sort(
                    key=lambda k: (k["runtime"] is None, k["runtime"]), reverse=reverse
                )
...
def import_trakt_list_tmdb(params):
    if not list_change_warning(params["list_name"]):
        return None
    from apis.trakt_api import get_trakt_list_selection
...
def process_trakt_list(chosen_list):
    ...
    if trakt_list_type in ("collection", "watchlist"):
        ...
        try:
            ...
            if sort_order == 0:
                result = sort_for_article(result, "title", ignore_articles())
            elif sort_order == 1:
...
def set_sort_order(list_id, sort_order):
    if tmdb_lists_cache.set_sort_order(list_id, sort_order):
        return True
...
def list_change_warning(
    ...
    return kodi_utils.confirm_dialog(
        heading="TMDb Lists", text=text % list_name, ok_label="Yes", cancel_label="No"
    )
