	...
	def _process():
		for item in episodes_data:
			try:
				...
				set_properties({
					'episode_type': episode_type, 'phage-lite.extras_params': extras_params, 'phage-lite.options_params': options_params,
...
def build_single_episode(list_type, params={}):
	def _get_category_name():
		try:
			cat_name = {'episode.progress': 'In Progress Episodes',
						...
						'episode.next_trakt': 'Next Episodes', 'episode.next_phage-lite': 'Next Episodes',
	...
	if list_type == 'episode.next':
		...
		else: resformat, resinsert, list_type = '%Y-%m-%d %H:%M:%S', '2000-01-01 00:00:00', 'episode.next_phage-lite'
