...
def get_personal_lists(params):
    def _process():
        for item in data:
            try:
                list_name, sort_order, description, list_total = (
                    item["name"],
                    item["sort_order"],
                    item["description"],
                    item["total"],
                ...
                if show_author:
                    name_insert = "%s | [I]%s[/I]" % (list_name, author)
                else:
                    name_insert = list_name
                if new_list_check(seen) and unseen_highlight:
                    display = "[COLOR %s]%s [I](x%02d)[/I][/COLOR]" % (
                        unseen_highlight,
                        name_insert,
                        list_total,
                    )
                else:
                    display = "%s [I](x%02d)[/I]" % (name_insert, list_total)
                custom_poster = item.get("poster", "")
                poster = custom_poster or icon
                custom_fanart = item.get("fanart", "")
                fanart = custom_fanart or background
                mode = (
                    "random.build_personal_lists_contents"
                    if random
                    else "personal_lists.build_personal_list"
                )
                url_params = {
                    "mode": mode,
                    "list_name": list_name,
                ...
                if random:
                    url_params["random"] = "true"
                if shuffle_lists:
                    url_params["shuffle"] = "true"
                url = build_url(url_params)
                cm = [
                    (
                        "[B]Make New List[/B]",
                        "RunPlugin(%s)"
                        % build_url({"mode": "personal_lists.make_new_personal_list"}),
                    ),
                    (
                        "[B]Edit Properties[/B]",
                        "RunPlugin(%s)"
                        % build_url(
                            {
                                "mode": "personal_lists.adjust_personal_list_properties",
                    ...
                    (
                        ...
                        % build_url(
                            {
                                "mode": "personal_lists.delete_personal_list",
                                "list_name": list_name,
                                "author": author,
                                "poster": custom_poster,
                                "fanart": custom_fanart,
                    ...
                    (
                        ...
                        % build_url(
                            {
                                "mode": "menu_editor.shortcut_folder_add_known",
                                "url": url,
                            }
                        ),
                    ),
    ...
    random, shuffle_lists = (
        params.get("random", "false") == "true",
        params.get("shuffle", "false") == "true",
    )
    ...
    try:
        ...
        if data:
            if shuffle_lists:
                returning_to_list = (
                    "build_personal_lists_contents" in kodi_utils.folder_path()
                )
                if returning_to_list:
                    ...
                    shuffle(data)
                    kodi_utils.set_property(
                        "bacterio.personal.lists.order", json.dumps(data)
                    )
            else:
                kodi_utils.clear_property("bacterio.personal.lists.order")
    ...
    if shuffle_lists and not returning_to_list:
        kodi_utils.focus_index(0)
...
def build_personal_list(params):
    ...
    try:
        ...
        list_name, author, sort_order = (
            params.get("list_name"),
            params.get("author"),
            params.get("sort_order"),
        )
        page_no, paginate_start = (
            int(params.get("new_page", "1")),
            int(params.get("paginate_start", "0")),
        )
        new_params = {
            "mode": "personal_lists.build_personal_list",
            "list_name": list_name,
            "author": author,
            "sort_order": sort_order,
            "seen": "true",
        ...
        if page_no == 1 and not is_external:
            kodi_utils.set_property("bacterio.exit_params", kodi_utils.folder_path())
        if use_result:
            result = params.get("result", [])
        else:
            result = get_personal_list(params)
        process_list, total_pages, paginate_start = _paginate_list(
            result, page_no, paginate_start
        )
        movie_list = {
            "list": [
                (c, i["media_id"])
                for c, i in enumerate(process_list)
                if i["type"] == "movie"
            ],
            "custom_order": "true",
        }
        ...
        content = (
            "movies"
            if len(movie_list["list"]) > len(tvshow_list["list"])
            else "tvshows"
        )
        ...
        if use_result:
            return content, [i[0] for i in item_list]
        kodi_utils.add_items(handle, [i[0] for i in item_list])
        if total_pages > 2 and settings.jump_to_enabled() and not is_external:
            kodi_utils.add_dir(
                handle,
                {
                    "mode": "navigate_to_page_choice",
                    "current_page": page_no,
                    "total_pages": total_pages,
                    "url_params": json.dumps(new_params),
                },
                "Jump To...",
                "item_jump",
                kodi_utils.get_icon("item_jump_landscape"),
                isFolder=False,
            )
    ...
    try:
        if sort_order:
            ...
            if sort_order in ("", "0", "None"):
                contents = sort_for_article(
                    contents, "name", settings.ignore_articles()
                )
            elif sort_order == "1":
                contents = sort_for_article(
                    contents, "author", settings.ignore_articles()
                )
            elif sort_order in ("2", "3"):
                reverse = sort_order != "2"
                contents.sort(
                    key=lambda k: (k["created_at"] is None, k["created_at"]),
                    reverse=reverse,
                )
            elif sort_order in ("4", "5"):
                reverse = sort_order != "4"
                contents.sort(
                    key=lambda k: k.get("updated", None) or "0", reverse=reverse
                )
...
def delete_personal_list(params):
    list_name, author, poster, fanart = (
        params.get("list_name", ""),
    ...
    if not kodi_utils.confirm_dialog(
        heading="Personal Lists", text="Delete [B]%s[/B] Personal List?" % list_name
    ):
        return
...
def delete_personal_list_contents(params):
    ...
    if not list_change_warning(list_name):
        return
    if personal_lists_cache.delete_list_contents(list_name, author):
        return
    kodi_utils.notification("Error Deleting List Contents", 3000)


def get_personal_list(params):
    list_name, author, sort_order, seen, update_seen = (
        params["list_name"],
        params["author"],
        params["sort_order"],
        params.get("seen", True),
        params.get("update_seen", True),
    )
    contents = personal_lists_cache.get_list(
        list_name, author, update_seen=update_seen, seen=seen
    )
    try:
        if sort_order == "None":
            pass
            ...
            contents.sort(key=lambda k: int(k["date_added"]), reverse=sort_order != "1")
        else:
            contents.sort(
...
def make_new_personal_list(params):
    is_retry, external_creation = (
        params.get("is_retry", False),
        params.get("external_creation", "false") == "true",
    )
    chosen_list, suggested_list_name, suggested_author = (
        params.get("chosen_list", []),
        params.get("suggested_list_name", ""),
        params.get("suggested_author", ""),
    )
    if (
        not external_creation
        and not is_retry
        and kodi_utils.confirm_dialog(
            heading="Personal Lists",
            text="Import a Trakt List to populate this new list?",
            ok_label="Yes",
            cancel_label="No",
        )
    ):
    ...
    if chosen_list:
        ...
        result = personal_lists_cache.add_many_list_items(
            list_name, author, new_contents
        )
    if not external_creation and any(
        [kodi_utils.path_check("get_personal_lists") or kodi_utils.external()]
    ):
...
def adjust_personal_list_properties(params):
    ...
    choices = [
        ("Change Name", "Currently [B]%s[/B]" % (list_name), "list_name"),
        ("Change Author", "Currently [B]%s[/B]" % (author), "author"),
        (
            "Change Sort Order",
            "Currently [B]%s[/B]" % sort_order_dict.get(sort_order, "None"),
            "sort_order",
        ),
        (
            "Change List Description",
            "Currently [B]%s[/B]" % (description),
            "description",
        ),
    ...
    if poster:
        choices.append(("Delete Custom Poster", "", "delete_poster"))
    if fanart:
        choices.append(("Delete Custom Fanart", "", "delete_fanart"))
    choices.extend(
        [
            (
                "Empty List Contents",
                "Delete All Contents of %s" % list_name,
                "empty_contents",
            ),
            (
                "Import Trakt List",
                "Import a Trakt List into %s" % list_name,
                "import_trakt",
            ),
        ]
    )
    ...
    kwargs = {
        ...
        "heading": "Personal List Properties",
        "multi_line": "true",
        "narrow_window": "true",
    }
    ...
    if action == None:
        return (
            kodi_utils.kodi_refresh()
            if params.get("refresh", "false") == "true"
            else None
        )
    if action in ("make_poster", "make_fanart"):
        ...
        shuffle_art = kodi_utils.confirm_dialog(
            heading="Personal Lists",
            text="Use [B]4 Random[/B] %s from List?[CR]OR[CR]Use [B]First 4[/B] %s from List?"
            % (art_type, art_type),
            ok_label="4 Random",
            cancel_label="First 4",
        )
        if shuffle_art == None:
    ...
    if action == "author":
        ...
        personal_lists_cache.update_single_detail(
            "author", new_author, list_name, author
        )
        ...
        if new_sort_order == None:
            return adjust_personal_list_properties(params)
        personal_lists_cache.update_single_detail(
            "sort_order", new_sort_order, list_name, author
        )
        params.update({"sort_order": new_sort_order, "refresh": "true"})
    elif action == "description":
        new_description = personal_list_description()
        personal_lists_cache.update_single_detail(
            "description", new_description, list_name, author
        )
        params.update({"description": new_description, "refresh": "true"})
    elif action == "make_poster":
        new_poster = personal_image_maker(
            list_name, author, "poster", sort_order, seen, poster, shuffle_art
        )
        if new_poster is None:
            return adjust_personal_list_properties(params)
        personal_lists_cache.update_single_detail(
            "poster", new_poster, list_name, author
        )
        params.update({"poster": new_poster, "refresh": "true"})
    elif action == "make_fanart":
        new_fanart = personal_image_maker(
            list_name, author, "fanart", sort_order, seen, fanart, shuffle_art
        )
        if new_fanart is None:
            return adjust_personal_list_properties(params)
        personal_lists_cache.update_single_detail(
            "fanart", new_fanart, list_name, author
        )
...
def personal_image_maker(
    ...
    if shuffle_art:
        shuffle(content)
    images = []
    api_key, mpaa, current_time, current_timestamp = (
    ...
    for item in content:
        if item["type"] == "movie":
            function = metadata.movie_meta
        else:
            function = metadata.tvshow_meta
        meta = function(
            "tmdb_id", item["media_id"], api_key, mpaa, current_time, current_timestamp
        )
        if meta.get(image_type):
            images.append(meta[image_type])
        if len(images) == 4:
            break
    final_image = make_image(
        "personal_lists", image_type, list_name, images, custom_image
    )
    kodi_utils.hide_busy_dialog()
    if final_image:
        personal_lists_cache.update_single_detail(
            image_type, final_image, list_name, author
        )
    return final_image


def personal_image_downloader(
    list_name, author, image_type, url, sort_order, seen, custom_image, show_busy=True
):
    if show_busy:
        kodi_utils.show_busy_dialog()
    content = get_personal_list(
    ...
    final_image = download_image(
        "personal_lists", image_type, list_name, url, custom_image
    )
    kodi_utils.hide_busy_dialog()
    if final_image:
        personal_lists_cache.update_single_detail(
...
def personal_list_name(list_name=""):
    new_name = kodi_utils.kodi_dialog().input(
        "Please Choose a Name for the New List", defaultt=list_name
    )
    if not new_name:
        return None
...
def personal_list_description():
    description = (
        kodi_utils.kodi_dialog().input("Optional Description for the New List") or " "
    )
    if not description:
        return None
...
def import_trakt_list(params):
    ...
    list_name, author, description, sort_order, seen = (
        params["list_name"],
        params["author"],
        params["description"],
        params["sort_order"],
    ...
    if result == "Success":
        if kodi_utils.confirm_dialog(
            heading="Personal Lists",
            text="Rename List to Match Trakt List Name?",
            ok_label="Yes",
            cancel_label="No",
        ):
            personal_lists_cache.update_single_detail(
                "name", trakt_list_name, list_name, author
            )
...
def process_trakt_list(chosen_list):
    ...
    trakt_list_type, trakt_list_name = (
        chosen_list.get("list_type"),
        chosen_list.get("name"),
    )
    if trakt_list_type in ("collection", "watchlist"):
        ...
        result = get_trakt_list_contents(
            trakt_list_type,
            chosen_list.get("user"),
            chosen_list.get("slug"),
            trakt_list_type == "my_lists",
        )
        try:
    ...
    for count, item in enumerate(result):
        try:
            ...
            if trakt_list_type in ("my_lists", "liked_lists") and item["type"] not in (
                "movie",
                "show",
            ):
                continue
            media_id = item["media_ids"]["tmdb"]
            if media_id in (None, "None", ""):
                continue
            title = item["title"]
            try:
            ...
            new_contents_append(
                {
                    ...
                    "title": title,
                    "type": media_type_check[media_type],
...
def external_process_view(params):
    def _process(function, _list, _type):
        if not _list.get("list", []):
            return
        data = function(_list).worker()
        results_extend(data)

    tmdb_movies, imdb_movies, tmdb_tvshows, imdb_tvshows = {}, {}, {}, {}
    item_list, list_name, list_type, media_type = (
        json.loads(params["item_list"]),
        params["list_name"],
        params["list_type"],
        params["media_type"],
    )
    movies = {
        "list": [(i["order"], i["id"]) for i in item_list if i["mt"] == "m"],
        "id_type": "%s_id" % list_type,
        "custom_order": "true",
    }
    tvshows = {
        "list": [(i["order"], i["id"]) for i in item_list if i["mt"] == "tv"],
        "id_type": "%s_id" % list_type,
        "custom_order": "true",
    }
    threads, results = [], []
    ...
    content = max(
        [
            ("movies", len(tmdb_movies.get("list", []) + imdb_movies.get("list", []))),
            (
                "tvshows",
                len(tmdb_tvshows.get("list", []) + imdb_tvshows.get("list", [])),
            ),
        ],
        key=lambda k: k[1],
    )[0]
...
class ExternalImport:
    def __init__(self, params, item_list):
        ...
        self.list_name, self.list_type, self.media_type = (
            params.get("list_name"),
            params.get("list_type", None),
            params.get("media_type", None),
        )
        self.action, self.author, self.description = (
            params.get("action", None),
            params.get("author", "Unknown") or "Unknown",
            params.get("description", ""),
        )
        self.import_indicator, self.poster, self.fanart = (
            params.get("busy_indicator", "none"),
            params.get("poster", ""),
            params.get("fanart", ""),
        )
        self.api_key, self.mpaa, self.current_time, self.current_timestamp = (
            settings.tmdb_api_key(),
            settings.mpaa_region(),
            get_datetime(),
            get_current_timestamp(),
    ...
    def process(self, item):
        try:
            ...
            function = (
                metadata.movie_meta if m_type == "movie" else metadata.tvshow_meta
            )
            meta = function(
                id_type,
                original_id,
            ...
            title, media_id, release_date, date_added = (
                meta["title"],
                meta["tmdb_id"],
                meta["premiered"],
                self.current_timestamp + order,
            )
            self.results_append(
                {
                    "media_id": str(media_id),
    ...
    def run(self):
        if self.import_indicator == "busy":
            kodi_utils.show_busy_dialog()
        elif self.import_indicator == "progress":
            self.progressDialog = kodi_utils.progress_dialog(
                "Importing Media", kodi_utils.get_icon("lists")
            )
            kodi_utils.sleep(1000)
        threads = TaskPool().tasks(
            self.process, self.item_list, min(self.total_items, settings.max_threads())
        )
        [i.join() for i in threads]
        self.results.sort(key=lambda k: k["order"])
        success = personal_lists_cache.make_list(
            self.list_name,
            self.author,
            "1",
            self.description,
            seen="true" if self.action == "import_view" else "false",
        )
        if not success:
            return kodi_utils.notification(
                "Error Creating [B]%s[/B]" % self.list_name, 3000
            )
        items_added = personal_lists_cache.add_many_list_items(
            self.list_name, self.author, self.results
        )
        if items_added == "Success":
            if self.poster:
                self.poster = personal_image_maker(
                    self.list_name,
                    self.author,
                    "poster",
                    "1",
                    "false",
                    "",
                    True if self.poster == "random" else False,
                    show_busy=False,
                )
            if self.fanart:
                if self.fanart not in ("first_4", "random"):
                    personal_image_downloader(
                        self.list_name,
                        self.author,
                        "fanart",
                        self.fanart,
                    ...
                    self.fanart = personal_image_maker(
                        self.list_name,
                        self.author,
                        "fanart",
                        "1",
                        "false",
                        "",
                        True if self.fanart == "random" else False,
                        show_busy=False,
                    )
            self.close_indicator()
            kodi_utils.notification(
                "Items Added to New List [B]%s[/B]" % self.list_name, 3000
            )
        else:
            self.close_indicator()
            kodi_utils.notification(
                "Error Adding Items to [B]%s[/B]" % self.list_name, 3000
            )
    ...
    def close_indicator(self):
        if self.progressDialog:
            ...
            kodi_utils.sleep(100)
        elif self.import_indicator == "busy":
            kodi_utils.hide_busy_dialog()
...
def external(params):
    ...
    list_name, list_type, media_type = (
        params.get("list_name"),
        params.get("list_type", None),
        params.get("media_type_default", None),
    )
    ...
    if not action:
        choices = [
            ("View", "view"),
            ("Import", "import"),
            ("Import & View", "import_view"),
        ]
        list_items = [{"line1": i[0]} for i in choices]
        kwargs = {
            "items": json.dumps(list_items),
            "narrow_window": "true",
            "heading": "TESTING: Choose Import Action",
        }
    ...
    if not item_list:
        return kodi_utils.notification("No Items in Import List. Try Again.", 3000)
    item_list = [
        dict(i, **{"order": c, "mt": i.get("mt") or media_type})
        for c, i in enumerate(item_list)
    ]
    if list_name:
    ...
    if not list_name:
        return
    if "import" in action:
        ExternalImport(params, [(i,) for i in item_list]).run()
    if "view" in action:
        kodi_utils.activate_window(
            {
                "mode": "personal_lists.external_process_view",
...
def unique_list_check(list_name, author="Unknown"):
    ...
    if list_name in list_names and author in list_authors:
        kodi_utils.notification(
            "List Already Exists. Choose a different name or author"
        )
        return False
    return True
...
def list_change_warning(
    list_name,
    text="[B]CAUTION!!![/B][CR][CR]This will change the contents of [B]%s[/B]. Continue?",
):
    return kodi_utils.confirm_dialog(
        heading="Personal Lists",
        text=text % list_name,
        ok_label="Yes",
        cancel_label="No",
